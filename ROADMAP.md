...
**Outcome (implemented, awaiting tag).** Every bullet above is done; the `inspect` one was re-scoped to [Phase 7.4](#phase-74-caller-chosen-output-dtype-remember-path) with its reason recorded in place. Four results worth carrying forward:
...
**Outcome (implemented, awaiting tag).** Every bullet above is done. Unlike Phase 7.3, whose `F32` cross-validation passed 22/22 on the first run, this phase's did **not**, and the failure was the most valuable thing in it. Six results worth carrying forward:
