...
- [3. Phased Development Plan](#3-phased-development-plan)
  ...
  - [Phase 7.7: Fixed-size output tiles across the dequant kernels](#phase-77-fixed-size-output-tiles-across-the-dequant-kernels)
...
**Outcome (implemented, awaiting tag).** All ten items are done, in eight
...
### Phase 7.7: Fixed-size output tiles across the dequant kernels

**Goal:** Finish the `chunks_exact` to `as_chunks` migration across the **five suppressed sites that remain in `src/`** after the `GPTQ` spike, on the mechanism that spike proved, and delete each suppression only when that site's own measurement licenses it. Ships as **`v0.7.7`**, which closes the 0.7 line. Every item here is a perf-claim commit under [`CLAUDE.md`](CLAUDE.md) *Performance Changes*, so every one needs before and after numbers, and any item whose number does not show a win is **closed as settled rather than shipped**.

**Provenance.** v0.7.6 shipped **six** `clippy::chunks_exact_to_as_chunks` suppressions under `src/`, plus fourteen more at file scope across the test and bench crates. Five of the six carried a real measurement: `+18 %` to `+21 %` on `dequant_fp8_fine_grained`, `+51 %` on `dequant_gptq_int4`, `+74 %` on `dequant_awq_int4`, `BnB` reverted with its twins despite a favourable `-7.6 %` reading that sat inside this host's noise floor, and [`src/remember/output.rs`](src/remember/output.rs) reverted with the family because the `FP8` regression could not be isolated away from the shared narrowing writer. **The sixth is the one this phase must not forget**: [`src/convert.rs`](src/convert.rs)'s `to_bf16_bytes`, the per-element narrowing every `bnb-nf4` conversion runs over a whole model, reverted with the rest and saying so plainly, because it has *no bench of its own* and unmeasured-plus-hot means unchanged. Those numbers were honest and reproducible. **What a reader would infer from them was wrong**, and the spike merged at `de9f390` (PR #3, 2026-08-21) is what showed it.

**The mechanism, now established rather than hypothesised.** Each tile loop zips four streams. Only three could migrate, because `as_chunks_mut::<{VECTOR_TILE * E::BYTES}>()` is rejected by stable Rust with *"generic parameters may not be used in const operations"*, so the output stayed a `ChunksExactMut` while the inputs became slices of arrays. **A homogeneous four-way zip became a mixed one, and that is what cost 51 % and 74 %, not `as_chunks`.** Giving `OutputElement` a fixed-size `Tile` associated type, with `split_tiles` and `write_tile` beside it, lets all four migrate together. The width is written as a literal per implementation (64 / 128 / 64) because generic code cannot compute it, and a `const` block proves each literal equals `VECTOR_TILE * BYTES` so the three cannot drift.

**The spike's numbers, which set the expectation for every item below.** CodSpeed macro runner (walltime, bare metal, isolated), `dequant_gptq_int4/synthetic_4096x11008_g128`, two runs per side:

| Kernel | main a | main b | spike a | spike b | Delta |
|---|---|---|---|---|---|
| **`dequant_gptq_int4`** (migrated) | 181.75 ms | 182.82 ms | 164.59 ms | 164.00 ms | **-9.87 %** |
| `dequant_awq_int4` (control) | 183.21 | 184.86 | 183.55 | 183.26 | -0.34 % |
| `dequant_bnb_int8` (control) | 161.09 | 159.73 | 158.35 | 159.59 | -0.90 % |
| `dequant_bnb_nf4` (control) | 183.64 | 185.03 | 187.10 | 184.90 | +0.90 % |
| `dequant_fp8_fine_grained` (control) | 203.56 | 203.97 | 204.98 | 207.19 | +1.14 % |
| `dequant_fp8_per_tensor` (control) | 199.50 | 200.01 | 200.96 | 196.51 | -0.51 % |

Run-to-run spread on the migrated kernel was **0.36 %**. Independent, layout-proof evidence agrees: reading `dequantize_gptq` at `--emit=asm`, x86-64 `target-cpu=native`, `opt-level=3`, instruction counts fell **1088 to 939** for `Bf16Out` (`-13.7 %`), **979 to 919** for `F32Out` (`-6.1 %`) and **1108 to 906** for `F16Out` (`-18.2 %`), with every documented vector family intact and `F32Out` *gaining* wide stores (`vmovups` 64 to 95, `%ymm` 96 to 160).

**The instrument, and why it can decide what this desktop cannot.** Two CodSpeed runs at `a665343` and `245f112` differ, under [`src/remember/`](src/remember/), by comments and attributes only, so six kernels are provably codegen-identical across them. They moved by **max 0.90 %, mean 0.59 %**. That is roughly 20x tighter than the development host, which [`CONVENTIONS.md`](CONVENTIONS.md) *Benchmark evidence* records reporting `+10.6 %` at `p = 0.00` on byte-identical source. **CodSpeed is the arbiter for every item below; the local best-of-N protocol remains the release gate.**

**And it runs `aarch64`, which the phase did not notice until item 1a.** Every `VECTORIZED:` annotation in this crate, and every instruction count quoted above including item 0's, was read on x86-64. The two are different targets and they disagree: on x86-64 the `AWQ` migration scalarises `F16`'s arithmetic, while on `aarch64` all three widths stay vectorised (`fmul` 14 vector / 2 scalar) before and after. From item 1 onward both are read, via `rustup target add aarch64-unknown-linux-gnu` plus `--emit=asm` (no linker needed for a lib-only build), and **`aarch64` decides**, because it is what the timing measures. x86-64 is still read because it is what the existing annotations claim and what the maintainer's machine runs; where they disagree, the comment says so rather than quoting the flattering one.

**One caveat, carried forward rather than buried.** `dequant_gguf_q4_k` improved **4.16 % consistently across both spike runs** without being touched. Consistent means it is a property of the binary rather than noise, but it is unattributed, most likely code layout or an inlining-budget shift from the three new trait methods. So the conservative floor on the `GPTQ`-specific gain is about **5.7 %**, not 9.87 %, and item 8 below exists to find out which.

- [x] **0. `GPTQ` itself, and the trait machinery the rest of the phase spends.** Done before the phase was written, and listed so the ledger is complete rather than because it is outstanding: the spike merged at `de9f390` landed `OutputElement::Tile` / `split_tiles` / `write_tile` and migrated [`src/remember/gptq.rs`](src/remember/gptq.rs)'s four streams, leaving that module with no suppression at all. **It shipped after the `v0.7.6` tag, so it releases as part of `v0.7.7`** and is already recorded under `## [Unreleased]` in [`CHANGELOG.md`](CHANGELOG.md). Items 1 to 5 spend the machinery it added — **done in `6550c26` / `b5be59b`, merged `de9f390`**
- [x] **1a. The benchmark suite could only see one of the three output widths.** *Not in the original plan; added when item 1 surfaced it, which is the whole reason item 1 went first.* `bench_awq_int4` calls `dequantize_awq_to_bf16`, and so did the `GPTQ`, `FP8` and `BnB` dequant groups: **every dequant benchmark in the suite was `BF16`-only**, so the phase's stated arbiter was blind to `F32` and `F16` entirely. That is not hypothetical. The `AWQ` migration improves `Bf16Out` by 16.1 % and `F32Out` by 8.6 % on instruction count while **scalarising** `F16Out`'s arithmetic (`vsubps` 11 to 6, scalar `vsubss` 2 to 41), and a `BF16`-only suite reports that as a clean win. It also means item 0's `-9.87 %` is a `BF16` number; `GPTQ`'s asm happens to be healthy at all three widths, so nothing was harmed, but that was luck rather than evidence. Fixed by giving all six migration-relevant groups `_f32` and `_f16` arms, plus an `_f16` arm for `dequant_gguf_q4_k`, whose output goes through the `write_scratch` that item 2 rewrites: **13 new ids, 9 to 22 in [`benches/dequant.rs`](benches/dequant.rs)**. The arms **record rather than gate** — `BF16` stays each group's regression guard, being the default width and the one every historical number is quoted at. `dequant_gguf_q4_k`'s standing note that `F16` needs no arm because it is *"the same width as `BF16`, so there is no bandwidth story to tell"* is **kept and answered in place** rather than deleted: it is still true about bandwidth, and these arms are about codegen — three monomorphisations are three codegen outcomes, and a suite that measures one measures one — **commit**
- [ ] **1. `AWQ`, the structurally identical twin.** [`src/remember/awq.rs`](src/remember/awq.rs) carries the same four-way tile loop as `GPTQ` and the same `MEASURED-REVERT`, at the largest half-migration cost of the five that were measured (`+74 %`). It needs no new trait machinery, since item 0 already landed `Tile` / `split_tiles` / `write_tile`. Migrate all four streams, delete the suppression, and confirm on `dequant_awq_int4` **at all three widths**, which is what item 1a exists to make possible. **Expected the largest single win of the phase**, and the cheapest to get: if `AWQ` does not reproduce something near `GPTQ`'s `-9.87 %`, the mechanism is not what the spike concluded, and the rest of this phase should stop and be re-thought rather than continue on faith — **commit**
- [ ] **2. `write_scratch` itself, the shared narrowing writer.** This is the hard one, and it is deliberately its own commit. [`src/remember/output.rs`](src/remember/output.rs) is what **every** dequant kernel funnels through, including the 24 `GGUF` kernel functions that reach it through the shared writer, so a change here moves every benchmark at once and nothing downstream can be attributed while it is in flight. It is also why the `FP8` regression could never be isolated: that `+18 %` to `+21 %` survived reverting both `FP8` loops individually. Each of the three `write_scratch` implementations carries a `// VECTORIZED: confirmed` annotation earned by reading its disassembly, so those three claims are re-established here, exactly as the spike re-established them for `write_tile`. Note that `write_scratch` is **not** deleted: the ragged edge run has a run-time length by construction, so the byte-slice form remains the correct shape for tails — **commit**
- [ ] **3. The `FP8` family.** [`src/remember/fp8.rs`](src/remember/fp8.rs), both the fine-grained and per-tensor paths, measured on `dequant_fp8_fine_grained` and `dequant_fp8_per_tensor`. Sequenced after item 2 because its recorded regression was never attributable to its own loops in the first place — **commit**
- [ ] **4. The `BnB` family.** [`src/remember/bnb.rs`](src/remember/bnb.rs), `NF4` and `INT8`. This one has a specific question attached: `INT8` measured `-7.6 %` locally when migrated, i.e. *faster*, and was reverted with its twins anyway because a reading that small is meaningless on a host with a 10 % noise floor. CodSpeed resolves 0.9 %, so the reading that was unusable in v0.7.6 becomes decidable here. Record the outcome either way — **commit**
- [ ] **5. `convert.rs::to_bf16_bytes`, which needs a benchmark before it can be decided at all.** The sixth suppressed site, and the only one whose comment admits it was reverted *without* evidence: *"this one has no bench of its own to clear it. Unmeasured plus hot means unchanged."* It is the per-element `F32`/`F16` to `BF16` narrowing that `convert --to bnb-nf4` runs over a whole model, and [Phase 7.4](#phase-74-caller-chosen-output-dtype-remember-path) made it *more* reachable by letting `--out-dtype f32` feed it from the hub. **So this item is two commits, not one:** add a `convert_to_bf16_bytes` group to [`benches/convert.rs`](benches/convert.rs) sized like its siblings, let CodSpeed take a baseline, and only then migrate and judge. Skipping the bench and migrating on the strength of items 1 to 4 would be exactly the inference this whole phase exists to stop making — **commit**
- [ ] **6. The fourteen test and bench crates, whose suppressions expire with the kernels.** Each carries a file-level `#![allow]` justified by a rationale that **inverts** the moment items 1 to 5 land: *"this code mirrors the kernel loops it exercises, and those kept `chunks_exact` on measured evidence."* Once a kernel migrates, the mirror is what now diverges. These are cold assertion loops, so there is nothing to measure and nothing to risk; sweep them last, when the kernels they mirror have settled, and delete the `#![allow(unknown_lints)]` guard alongside wherever it was carried only for this lint — **commit**
- [ ] **7. Correct what v0.7.6 shipped, including one contradiction between two of its own comments.** The suppression comments are live on crates.io stating costs of `+18 %` to `+74 %` for migrating a loop to `as_chunks`. Each is true as a measurement and **misleading as an inference**, because what was measured was a *half* migration. Every suppression this phase deletes takes its comment with it; any that survives on its own number is reworded to say *which* shape it measured. **And the two `FP8` accounts must be reconciled rather than both deleted quietly:** [`src/remember/fp8.rs`](src/remember/fp8.rs) says the regression was *"isolated to this module rather than to one function"*, while [`src/remember/output.rs`](src/remember/output.rs) says the same regression *"could not be isolated away from here"*. They name different culprits, they shipped together, and item 3's measurement is what decides which was right — **commit**
- [ ] **8. Attribute the unattributed `4.16 %`.** `dequant_gguf_q4_k` moved consistently without being touched. Two candidate explanations, and they are distinguishable: pure code layout, or an inlining-budget shift caused by adding three methods to a trait every kernel instantiates. Compare instruction counts for one `GGUF` block kernel across `245f112` and `de9f390` the way the spike did for `dequantize_gptq`, since an instruction-count change cannot be layout. **If it is real, it belongs in [`CHANGELOG.md`](CHANGELOG.md) as a `GGUF` win rather than being quietly banked**; if it is layout, the phase's other numbers need the same 4 % haircut applied to them honestly — **commit**
- [ ] **9. `docs/perf-experiments.md` rows, including the negative and null results.** One row per item above, per that file's standing rule that an attempted experiment is recorded even when the result is "no change" or a regression. Item 0 gets the first row, since it is the experiment that reversed a shipped conclusion — **commit**
- [ ] **10. Two [`CONVENTIONS.md`](CONVENTIONS.md) amendments the spike paid for.** **(a)** A `MEASURED-REVERT` must state *what shape was measured*, because "the lint's advice is slower" and "a partial application of the lint's advice is slower" are different claims, and only the second was ever true here. **(b)** Record the CodSpeed noise floor (`0.90 %` max over six codegen-identical kernels) beside the existing *Benchmark evidence* section's local figure, so the next reader knows which instrument can license which size of claim — **commit**
- [x] **11. The benchmark suite's own finding — and it was worse than "flat".** *Promoted ahead of the kernel items, because it was corrupting the signal they are judged by.* The file-writing groups cannot see the parallel path: `remember_gguf_whole_model` reads `167.45 ms` at 1 thread against `168.47 ms` at 4, and `convert_gguf_to_safetensors` reads `315.39 ms` at **both** budgets at `F32`, to the last decimal. That much [Experiment 12](docs/perf-experiments.md) predicted and `bench_remember_gguf_whole_model`'s own doc comment asked to have recorded (*"in which case say so here rather than trusting it"*). **What the phase found on top is that their two measurement bases disagree by about 2x**: criterion's own median is stable across five consecutive `main` runs at `168.32`-`168.47 ms`, while CodSpeed's reported `BASE` for the same id is `89.9 ms`. Every other group agrees between the two to within 1 %. Together those made CodSpeed report *"degrade performance by 34.31 %"* on [PR #4](https://github.com/mi-for-the-rust-of-us/anamnesis/pull/4), which changed no `src/` file at all. **A guard that fires on changes it cannot see is worse than no guard**, because it teaches the reader to click past a red check — and items 1 to 5 are each judged by exactly that check. Both groups now sit behind a new `bench-fileio` feature that [`.github/workflows/codspeed.yml`](.github/workflows/codspeed.yml) deliberately does not enable, so they still run under `cargo bench --all-features` where the storage is the maintainer's own. The `GGUF` conversion keeps a real CI guard in `convert_bytes_gguf_to_safetensors`, which goes `90.70` to `19.63 ms` — **4.62x** — on the same runner where its file-writing sibling reports `1.00x` — **commit**
- [ ] **12. `F16` output costs 2.1x to 2.9x `BF16` at the same width, and nothing says so.** *Found by item 1a's own arms, within minutes of their first baseline.* `F16` and `BF16` are both 2 bytes per element, so this is not the bandwidth story `F32` tells; it is a missed inline. On `aarch64`, `Bf16Out::write_scratch` and `F32Out::write_scratch` inline into their callers while **`F16Out::write_scratch` does not** — it is an out-of-line `bl` per 32-element tile (3 calls in `AWQ`, 1 in the already-migrated `GPTQ`, which is itself evidence the migration helps `F16` on the platform that counts). Measured at the first baseline: `dequant_gptq_int4` `162.81` / `245.60` / `476.60 ms` at `BF16` / `F32` / `F16`, and the same shape across all seven groups. **Two deliverables, and the second ships even if the first fails:** (a) find out whether the inline can be recovered, noting that `#[inline(always)]` is already ruled out — item 1 tested it and it *scalarises `BF16`* in both kernels; (b) document the cost on [`F16Out`](src/remember/output.rs) regardless. Its doc today discusses range and precision at length and says nothing about speed, while `F32Out`'s correctly warns that it doubles output bytes. A `pip install` audience choosing `f16` for its accuracy is currently paying 2x to 3x for it silently — **commit** — **PUSH + tag `v0.7.7`**

**Deliverable:** `anamnesis` v0.7.7 — the dequant kernels use fixed-size output tiles throughout, `clippy::chunks_exact_to_as_chunks` is either satisfied or suppressed on a number that names the shape it measured, and the crate no longer ships comments implying a conclusion its own follow-up reversed. [Phase 8](#phase-8-python-bindings-pyo3) then binds kernels that are both faster and lint-clean. — **PUSH + tag `v0.7.7`**

**Explicitly out of scope for v0.7.7:**

- **The `GGUF` block kernels' own chunking.** [`src/remember/gguf.rs`](src/remember/gguf.rs) migrated at v0.7.6 across 27 sites, all at the concrete width `2`, and is already lint-clean; so are the two wider `as_chunks_mut::<34>` / `::<144>` sites in [`src/convert.rs`](src/convert.rs), which are a different loop from the `to_bf16_bytes` one item 5 covers. Item 2 changes what these *call*, not how they chunk.
- **Anything that changes output bytes.** Every item is a codegen-shape change behind unchanged arithmetic. The `cross_validation_*` suites are the gate, and a byte difference is a defect in the migration rather than a decision to be made.
- **Raising `VECTOR_TILE`.** 32 is measured: an 8-element tile left more loop-setup overhead per element than the register pressure it saved. Fixed-size tiles make a different value cheaper to try, but that is a separate experiment with its own row.
