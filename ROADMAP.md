...
- [x] **`benches/convert.rs` (CodSpeed)** — whole-model groups at **1 and 4 threads**, closing the gap where Phase 7's headline speedup had no continuous regression guard; plus a **TSan CI job**, satisfying the `CONVENTIONS.md` race-detector rule that Windows/MSVC could never satisfy locally — **commit** — **PUSH + tag `v0.7.2`**
