...
- [3. Phased Development Plan](#3-phased-development-plan)
  ...
  - [Phase 7.3: Caller-Chosen Output Dtype](#phase-73-caller-chosen-output-dtype)
...
### Phase 7.3: Caller-Chosen Output Dtype

**Goal:** Make the dequantisation output type a **caller-chosen parameter** — `BF16`, `F32`, `F16`, and whatever comes later — instead of the hard-coded `BF16` the crate has emitted since its first commit. Ships as **`v0.7.3`**, deliberately *before* [Phase 8](#phase-8-python-bindings-pyo3).
...
Together with the day-one `#[non_exhaustive]`, that is **three separate pieces of scaffolding for output dtypes that never arrived**. They are all still in place and still load-bearing for exactly one variant: `FromStr` today has a single arm (`"bf16" =>`) and an error string reading *"supported target dtypes: bf16"*. Phase 7.3 is therefore less "add a feature" than **"hang something on the hooks the crate has carried since its second day"** — which is also why the plumbing cost below is modest.
...
**The architecture already fits this — the cost is far lower than it looks.** All 22 `GGUF` kernels write into an `&mut [f32]` scratch and **never mention `BF16`**; the narrowing happens at exactly **two call sites**, both calling the single `write_scratch_to_bf16` ([`src/remember/gguf.rs`](src/remember/gguf.rs)), with `f32_bits_to_bf16_bits` ([`src/remember/fp8.rs`](src/remember/fp8.rs)) playing the same role for the other families. **The `f32` values already exist and are already exact** — `F32` output means *not narrowing*. Making the tiny writer generic (rather than the kernels) also bounds monomorphisation to one small function per output type instead of 22 kernels × N types.
...
2. **It de-risks Phase 8.** `NumPy` has native `float32` and `float16` but **no** `bfloat16`; the frozen `ml_dtypes` return contract exists solely to work around a limitation that an `F32`/`F16` output removes. A Python user who wants a plain `np.float32` array — i.e. most of them — would get one with no optional dependency.

- [ ] **`OutputElement` trait + generic `write_scratch`**, with `Bf16Out` / `F32Out` / `F16Out`; kernels untouched; existing `dequantize_*_to_bf16` entry points retained as thin `::<Bf16Out>` wrappers so nothing breaks — **commit**
- [ ] **`TargetDtype::{F32, F16}` + the boundary dispatch** — one `match` instantiating the generic; `ConvertTarget` / hub / `remember` plumbing taught that output width is no longer 2 — **commit**
- [ ] **CLI `--to f32` / `--to f16`** and the `# Memory` doc sections, which all currently hard-code `n_elements × 2`; the `dhat` peak-heap assertions in `tests/peak_heap_*.rs` are likewise `BF16`-width — **commit**
- [ ] **Cross-validation at `F32`, compared directly against `gguf-py` with no rounding step** — the stronger claim above; extend the fixture generators to store `float32` goldens — **commit**
- [ ] **Measure and record — but this is a capability, not a perf claim.** `F32` doubles output bytes on a path Experiment 12 showed is bandwidth-bound, so expect it to be **slower** than `BF16` and expect the Phase 7.2 threading ratios to shift. **That is not a blocker.** The [`CLAUDE.md`](CLAUDE.md) "no measured win → no commit" rule governs *perf-claim* commits; Phase 7.3 claims **exactness**, not speed, and a correct-but-slower `F32` path is the point of it. Record the numbers in `docs/perf-experiments.md` as the honest cost of the new option, then optimise it as its own follow-up rather than gating the capability on it. Also verify binary-size growth from monomorphisation, and quantify `F16`'s accuracy gain (11-bit significand vs `BF16`'s 8, and dequantised weights sit well inside `f16` range — it is plausibly the better *2-byte* choice for this data) — **commit** — **PUSH + tag `v0.7.3`**

**Deliverable:** `anamnesis` v0.7.3 — the output dtype is a caller-chosen parameter, `F32` recovers the **exact** dequantised value for the first time, and the Python bindings arrive at a settled dtype contract instead of freezing a `BF16`-only one. — **PUSH + tag `v0.7.3`**
