...
- [ ] **11. The benchmark suite's own finding, acted on.** CodSpeed shows `remember_gguf_whole_model` at `167.45 ms` on 1 thread against `168.47 ms` on 4, and `convert_gguf_to_safetensors` at `315.39 ms` on both at `F32`: **the file-writing benchmarks are storage-bound on macro runners and cannot see the parallel path at all**, exactly as [Experiment 12](docs/perf-experiments.md) predicted and as `bench_remember_gguf_whole_model`'s own doc comment asked to have recorded (*"in which case say so here rather than trusting it"*). `convert_bytes_gguf_to_safetensors` meanwhile goes `90.70` to `19.63 ms`, a real **4.62x**. Say so in both bench doc comments, and stop treating the flat pair as a threading guard — **commit** — **PUSH + tag `v0.7.7`**
