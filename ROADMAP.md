...
**Goal:** Give every verb the crate exposes **one shape across all four formats**, before [Phase 8](#phase-8-python-bindings-pyo3) freezes those shapes into a PyPI signature. Ships as **`v0.7.6`**, the last Rust-only tag of the 0.7 line. Nothing here is a new capability: every item is an asymmetry the API grew while each format was added one phase at a time, plus one measured defect that ships today.
...
**Goal:** Finish the `chunks_exact` to `as_chunks` migration across the five kernel families [Phase 7.6](#phase-76-api-parity-before-the-bindings) could not take it to, on the mechanism the `GPTQ` spike proved, and delete each suppression only when that kernel's own measurement licenses it. Ships as **`v0.7.7`**, the genuine last tag of the 0.7 line. Every item here is a perf-claim commit under [`CLAUDE.md`](CLAUDE.md) *Performance Changes*, so every one needs before and after numbers, and any item whose number does not show a win is **closed as settled rather than shipped**.

**Provenance.** v0.7.6 shipped five `clippy::chunks_exact_to_as_chunks` suppressions, each carrying a real measurement: `+18 %` to `+21 %` on `dequant_fp8_fine_grained`, `+51 %` on `dequant_gptq_int4`, `+74 %` on `dequant_awq_int4`, `BnB` reverted with its twins despite a favourable `-7.6 %` reading that sat inside this host's noise floor, and [`src/remember/output.rs`](src/remember/output.rs) reverted with the family because the `FP8` regression could not be isolated away from the shared narrowing writer. Those numbers were honest and reproducible. **What a reader would infer from them was wrong**, and the spike merged at `de9f390` (PR #3, 2026-08-21) is what showed it.
...
- **The `GGUF` block kernels' own chunking.** The 24 kernels in [`src/remember/gguf.rs`](src/remember/gguf.rs) migrated at v0.7.6 on concrete block widths (34, 144, 2) and are already lint-clean. Item 2 changes what they *call*, not how they chunk.
