...
> **⚠️ Scoped into two tags by an audit finding, not by size alone.** The original single-phase plan assumed the cheap `GGUF` path and the `TargetDtype` public API were the same piece of work. They are disjoint. [`QuantScheme`](src/parse/safetensors.rs) shows `ParsedModel::remember`, the only consumer of `TargetDtype`, covers `FP8` / `GPTQ` / `AWQ` / `BnB4` / `BnbInt8` and **never reaches a `GGUF` kernel**: `GGUF` dequant is reachable only through `convert` ([`src/convert.rs`](src/convert.rs)) and the CLI, neither of which accepts an output-dtype parameter at all today.
>
> So the phase's *motivation* (the exactness table below, measured entirely on `gguf-py`) sits on one path and the API it is named after sits on the other. **v0.7.3 takes the `GGUF` / `convert` side**: the shared writer, the 24 kernels that come with it for free, a new output-dtype parameter on `ConvertOptions`, and the direct `F32`-vs-`gguf-py` cross-validation that realises the whole exactness claim. **[Phase 7.4](#phase-74-caller-chosen-output-dtype-remember-path) takes the `remember` side**: `TargetDtype::{F32, F16}` and the four families whose kernels fuse the narrowing into the loop body and therefore cannot reuse the writer.
...
**80–97 % of dequantised values are not `BF16`-representable and are rounded.** The arithmetic is forced: a `Q8_0` value is an `f16` scale (11-bit significand) × an `int8`, needing up to ~18 bits; `BF16` holds 8, `F32` holds 24. **Only `F32` is exact.** The max relative error is exactly `2⁻⁸` — half a `BF16` `ULP`, as it must be.
...
2. **Exact in the real-number sense.** True for the single-multiply kernels: `Q8_0` and `Q5_0` (`f16` scale, 11 significand bits, times a small int) and `Q6_K` (11 + 7 + 6 = 24 bits, landing exactly on `f32`'s 24-bit significand at both steps). **Not** guaranteed for the min-offset K-quants `Q2_K` / `Q4_K` / `Q5_K`, whose final `d·q - dmin·m` subtracts two `f32` values at different exponents and can itself round. There, `f32` is the reference's own value, not a mathematically exact one.

The deliverable is therefore worded as *removing a narrowing step*, not as *recovering the true value* for every kernel.

**Design: a template parameter, static inside and dynamic at the edge.** The output element type becomes a generic parameter, monomorphised exactly like a C++ template type argument, with a single runtime `match` at the public boundary so the CLI (`--to f32`) and the Phase 8 bindings (a dtype string) can still choose at run time:
...
1. **A strictly stronger correctness claim.** Today the cross-validation must round gguf-py's `float32` down to `BF16` before comparing. With `F32` output the comparison is **direct, with no rounding step in between** — bit-exact against the reference as the reference actually produces it.
...
- [ ] **`OutputElement` trait + generic `write_scratch`**, with `Bf16Out` / `F32Out` / `F16Out`, replacing `write_scratch_to_bf16`. All 24 `GGUF` kernel functions untouched; `dequantize_gguf_to_bf16` / `dequantize_gguf_blocks_to_bf16` retained as thin `::<Bf16Out>` wrappers so nothing breaks — **commit**
...
- [ ] **The streaming sink contract becomes dtype-dependent.** `dequantize_gguf_blocks_to_bf16` documents its sink as receiving 64 B per legacy block and 512 B per K-quant super-block; those become `QK × E::BYTES`. Public doc updated plus a test asserting the observed block length per output type, so a caller's `chunks_exact(2)` assumption fails loudly rather than silently truncating. *(Gap the original single-phase plan did not name.)* — **commit**
- [ ] **Output dtype on the `convert` path — the parameter that does not exist yet.** `ConvertOptions::output_dtype` / `with_output_dtype` as a builder (matching `with_threads`), threaded through `read_gguf` / `read_hub` and both writers, replacing the hard-coded `Dtype::BF16` in [`src/convert.rs`](src/convert.rs) and [`src/cli.rs`](src/cli.rs); CLI `--out-dtype bf16|f32|f16` on `convert`. Without this the new generic writer has **no caller**, because `TargetDtype` does not reach the `GGUF` path — **commit**
- [ ] **Passthrough policy, stated rather than assumed.** `remember` and `convert` already emit mixed-dtype files: dequantised tensors are `BF16`, passthrough tensors keep their source dtype ([`src/model.rs`](src/model.rs)). Decide and document that an `F32` request widens **dequantised tensors only** and leaves passthrough untouched, so nobody reads `--out-dtype f32` as "rewrite every tensor as `F32`" — **commit**
...
**Goal:** Finish what [Phase 7.3](#phase-73-caller-chosen-output-dtype-gguf--convert-path) starts by extending the caller-chosen output dtype to `ParsedModel::remember`, which means filling in `TargetDtype::{F32, F16}` and making the four quantisation families that `remember` actually dispatches over (`FP8`, `GPTQ`, `AWQ`, `BnB`) generic over `OutputElement`. Ships as **`v0.7.4`**, still ahead of [Phase 8](#phase-8-python-bindings-pyo3), so the Python bindings freeze a settled dtype contract on **both** paths rather than a `BF16`-only one.

**Why this is the larger half.** Phase 7.3's cost is low because `GGUF` funnels 24 kernels through one shared writer. These four families have no such funnel: each calls `f32_bits_to_bf16_bits` from inside its own hot loop, and each computes output offsets in units of 2 bytes at every level of its loop nest. There are six non-test narrowing sites, and generalising them means editing kernel bodies, not a single helper. The `f32` values still exist and are still computed the same way, so the *idea* carries over unchanged; only the "one small function" economy does not.
...
- [ ] **Output-side plumbing** — `build_views` hard-codes `safetensors::Dtype::BF16` for every dequantised tensor ([`src/model.rs`](src/model.rs)); CLI `--to f32` / `--to f16` on `remember`; the same passthrough policy Phase 7.3 documents for `convert`, applied and stated here — **commit**
- [ ] **`# Memory` doc sections and `dhat` assertions made dtype-parameterised.** Every one currently reads `2 × n_parameters` or `total_elements * 2` (three `# Memory` sections on `remember`, plus `tests/peak_heap_awq.rs`, `tests/peak_heap_gptq.rs`, and `tests/peak_heap_bnb_dq.rs`, two assertions each). [`src/lib.rs`](src/lib.rs) promises these are verified to the byte on the reference, so they have to be re-derived per output type, not merely reworded — **commit**
- [ ] **Cross-validation at `F32` for all four families**, against the same Python references the `BF16` suites already use, with no rounding step in between. Same caveat as Phase 7.3: rounding to `BF16` has been hiding 16 mantissa bits of any disagreement, so treat first-run failures as real findings rather than test bugs — **commit**
