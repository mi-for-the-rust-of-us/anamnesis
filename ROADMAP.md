...
- [x] **12. `F16` output costs 2x to 3x `BF16` at the same width — on BOTH architectures, which the plan had wrong.** This item was scoped as an `aarch64` finding with Apple Silicon as the open question. Measuring x86-64 with the paired harness shows **the same penalty there**: `gptq_int4` 27.5 to 81.8 ms (**2.97x**), `bnb_int8` 24.1 to 72.2 ms (**3.00x**), `awq_int4` **2.69x**, `bnb_nf4` **2.30x**, `fp8_per_tensor` **2.19x**, `fp8_fine_grained` **2.09x**, `gguf_q4_k` **1.99x**, against `aarch64`'s 2.10x-2.93x on the same seven kernels. **Deliverable (a) is answered in the negative, and usefully so:** the `aarch64` inlining failure is *not* the lever, because x86-64 already has the inline and still pays 2x to 3x. Two different mechanisms give the same magnitude — x86-64's `vcvtps2ph` takes a 128-bit source, so the narrowing runs **4 lanes** where `BF16`'s bias-and-shift runs **8**; `aarch64` simply does not inline `F16Out::write_scratch` at all. The conversion is the cost, not the call, so no cheap fix exists and none is attempted. **Deliverable (b) ships:** [`F16Out`](src/remember/output.rs) gains a `# Cost` section with the table, both mechanisms, and the Apple Silicon question left explicitly open. [`F32Out`](src/remember/output.rs) is quantified in passing too — it costs **1.10x to 1.92x**, *less* than its 2x byte ratio implies, because it skips the narrowing `BF16` performs — **commit** — **PUSH + tag `v0.7.7`**
