...
- [ ] **12. `F16` output costs 2.1x to 2.9x `BF16` at the same width on `aarch64`.** Both operands are 2 bytes, so this is not `F32`'s bandwidth story; on that target `Bf16Out::write_scratch` and `F32Out::write_scratch` inline while **`F16Out::write_scratch` does not**. **Re-scoped by the architecture finding**: this was measured on Linux `aarch64` server hardware and has **not** been confirmed on Apple Silicon, which has hardware `FP16` arithmetic and may not show it at all. So: measure on x86-64 with `ab.rs`, keep the `aarch64` number, state the Apple Silicon question as open, and **document the cost on [`F16Out`](src/remember/output.rs) regardless** — its doc discusses range and precision at length and says nothing about speed, while `F32Out`'s correctly warns about output bytes — **commit** — **PUSH + tag `v0.7.7`**
