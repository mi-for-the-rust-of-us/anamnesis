...
- [ ] **6. The fourteen test and bench crates.** Their file-level allows rest on *"this code mirrors the kernel loops it exercises, and those kept `chunks_exact` on measured evidence"*. That rationale now varies per kernel, so each sweep follows its kernel's verdict — **commit**
- [ ] **7. Correct what v0.7.6 shipped, including a contradiction between two of its own comments.** [`src/remember/awq.rs`](src/remember/awq.rs) is done (item 1); the rest follow their own numbers. And the two `FP8` accounts must be reconciled rather than both deleted quietly: `fp8.rs` says the regression was *"isolated to this module rather than to one function"* while `output.rs` says it *"could not be isolated away from here"*. They name different culprits and shipped together — **commit**
...
- [ ] **9. `docs/perf-experiments.md` rows, including the negatives.** The `AWQ` reversal gets the first row: it is the experiment that overturned a shipped conclusion *and* retired an instrument — **commit**
- [ ] **10. [`CONVENTIONS.md`](CONVENTIONS.md), which this phase has rewritten the evidence for.** Four amendments, in descending order of how much they cost to learn: **(a)** a static instruction count never decides a perf question, on any architecture, with the `AWQ` numbers as the worked example; **(b)** every instrument must state its measured floor on identical code, with the three floors tabulated above; **(c)** a `MEASURED-REVERT` must say *what shape* was measured, since "the lint's advice is slower" and "half the lint's advice is slower" are different claims; **(d)** the architecture a number came from is part of the number — **commit**
