...
- [ ] **2. `write_scratch`, the shared narrowing writer.** Its own commit, because [`src/remember/output.rs`](src/remember/output.rs) is what every dequant kernel funnels through, so nothing downstream is attributable while it is in flight. Add its callers' arms to `ab.rs` first, since a change here moves all of them at once — **commit**
- [ ] **3. The `FP8` family.** [`src/remember/fp8.rs`](src/remember/fp8.rs), fine-grained and per-tensor. Note the shape difference that the re-scope makes relevant: `FP8`'s hot loop zips **two** streams, not four, so the mechanism that explains `GPTQ` and `AWQ` may not apply here at all — **commit**
- [ ] **4. The `BnB` family.** [`src/remember/bnb.rs`](src/remember/bnb.rs), `NF4` and `INT8`, also two-stream. `INT8` measured `-7.6 %` locally in v0.7.6 (i.e. *faster*) and was reverted with its twins because a reading that small was meaningless at a 10-23 % floor. At ~2 % it becomes decidable — **commit**
- [ ] **5. `convert.rs::to_bf16_bytes`.** The sixth suppressed site, and the only one whose comment admits it was reverted *without* evidence. The v0.7.6 plan called for a whole `convert_bytes` path benchmark; that is no longer necessary, because `ab.rs` can call the narrowing directly at the same cost as any other arm — **commit**
