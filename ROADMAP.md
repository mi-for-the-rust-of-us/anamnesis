...
- [3. Phased Development Plan](#3-phased-development-plan)
  ...
  - [Phase 7.6: API parity before the bindings](#phase-76-api-parity-before-the-bindings)
  - [Phase 7.7: Settle each suppressed site on its own measurement](#phase-77-settle-each-suppressed-site-on-its-own-measurement)
...
- **CVE assignment.** anamnesis hasn't shipped a publicly-known exploitable issue under its own name; the candle #3533 disclosure is upstream. Internal hardening before public Python exposure ([Phase 8's](#phase-8-python-bindings-pyo3) `pip install anamnesis-quant`) is the right framing — there's no CVE to assign because there's no known in-the-wild attack. If a downstream consumer reports active exploitation against a pre-v0.6.1 deployment, revisit.
...
**Deliverable:** `anamnesis` v0.6.3 — the security core (caller-configurable `ParseLimits` per-item + aggregate, a decompression-ratio cap, the documented inspect-before-parse policy gate) **plus** the `remember_to_bytes` in-memory dequant companion (a non-security `Added` that rides in the same tag so we don't ship a one-method release) **plus** the inflate-only `zip` dependency tightening (drops the unused `zopfli` compressor — a non-security `Changed`). Default behaviour unchanged; no breaking change. The `CHANGELOG` carries a `### Security` section (the governance work), an `### Added` section (`remember_to_bytes`), and a `### Changed` section (inflate-only `zip`), keeping the security narrative honest. Lands before Phase 8 so `pip install anamnesis-quant` exposes an already-governed parser. — **PUSH + tag `v0.6.3`**
...
- [x] **`copy_to_contiguous` flat-index pass-2 conversion** — **dropped.** It was SIMD-plan scaffolding; the path is rare (<0.1 % of `state_dict` files), there is no measured need, and Phase 7 shipped no SIMD. Revisit only if a real hot-path case surfaces.
- [x] **AWQ/GPTQ pass-2 zip-chain audit** — **done (Experiment 10):** the pass-2 arithmetic *already* partially auto-vectorises (`vsubps`/`vmulps` in the index-0 disassembly of both kernels); a naive fission of the loop *regressed* it to scalar and was reverted. No refactor shipped — the auto-vectoriser already achieves what this item hoped for.
...
- [x] **GGUF-reader parallelisation** — **moved to [Phase 7.2](#phase-72-gguf-reader-parallelisation--fine-tuning) (v0.7.2)**: `read_gguf`'s own per-tensor loop over mmap-borrowed `ParsedGguf` needs a loop restructure before it can join the scoped-thread pool. Deferred so v0.7.0 ships the done-and-verified parallel dequant now. *(This bullet originally also cited `ParsedGguf: Sync` as a prerequisite; Phase 7.2 measured that it was already satisfied — see that phase's banner.)*
...
1. **GGUF-reader parallelisation** (deferred from Phase 7). The `convert` GGUF-*input* path (`read_gguf`) dequantised tensors single-threaded and did not even receive the caller's thread budget. Bring it under the same per-tensor parallel dispatch — same `ConvertOptions.threads` budget, same determinism contract (byte-identical across thread counts).
...
> **⚠️ Two planning assumptions were corrected by measurement.**
> **(a) `ParsedGguf: Sync` was never a blocker.** This phase was scoped around needing it; a compile-time assertion showed it was *already* satisfied — every field (`Backing`, the metadata `HashMap`, the tensor-info `Vec`) is `Sync`, and `memmap2::Mmap` is `Send + Sync`. No `unsafe`, no marker impl. `tests/parallel_contract.rs` now asserts it so a future field addition fails by name rather than as an inscrutable closure bound.
> **(b) The win is ~1.9×, not the ~3–4× the safetensors path enjoys** ([`docs/perf-experiments.md`](docs/perf-experiments.md) Experiment 12). The hub owns a `Vec<u8>` **per tensor**, which is structurally Experiment 11's *`Vec`-per-task* case — and that experiment measured its ceiling at 2.2×, versus 2.9× for pre-allocated disjoint slices. The reader lands exactly there. Going faster means redesigning `Hub` toward one contiguous model-wide buffer: a memory-layout change with an API blast radius that also disturbs the Phase 6.14 copy-elimination work, so it is recorded as the known ceiling rather than attempted here. End-to-end `convert()` gains less again (1.19–1.36×) because writing the multi-`GiB` output is ~60 % of the wall clock and is inherently sequential — **Amdahl, not a defect**.

- [x] **Shared parallel dispatch (`src/parallel.rs`)** — one `map_indexed` helper replacing v0.7.0's inline `thread::scope` block, claiming work through an **atomic cursor** instead of contiguous equal-count chunks (GGUF tensor sizes are too skewed for a static split); index-tagged + sorted results, so ordering is independent of steal order *and* thread count; deterministic lowest-index error selection matching the sequential loop. Both the safetensors and GGUF paths now share one `// PARALLEL:` site — **commit**
- [x] **Measured `MIN_PARALLEL_BYTES` (4 MiB)** — the named size floor `CONVENTIONS.md` rule 3 requires and v0.7.0 left implicit. Calibrated, not guessed: a 4-worker scoped pool costs **236 µs** to spawn+join on Windows against ~1.05 GB/s of single-thread reader throughput → break-even ~0.5 MiB, taken with an ~8× margin — **commit**
- [x] **Parallel `read_gguf` + budget threaded through `read_hub`**, determinism-tested across `{1, 2, 4, 8, 16}` and the resolved default, for the `safetensors` / `gguf` / `bnb-nf4` targets — **commit**
- [x] **First quantised-GGUF convert coverage** — a coverage hole this phase surfaced: every existing GGUF-input convert test builds its fixture with `write_gguf`, which *rejects* quantised dtypes, so the branch being parallelised was exercised only against the gitignored multi-GiB local models and **never in CI**. Closed with a hand-rolled quantised-GGUF writer + a 17-tensor mixed fixture checked against the kernel directly — **commit**
- [x] **API/doc fine-tuning ahead of the bindings** — `RememberOptions::new()` + `with_threads` as a **builder** (breaking; aligns with `ConvertOptions`, one-line migration), `remember_with_progress_and_options` (progress *and* a budget were previously mutually exclusive), CLI `--threads` on `remember` / `convert` (the budget was unreachable from the CLI), and a corrected `ConvertOptions::threads` doc — **commit**
- [x] **`benches/convert.rs` (CodSpeed)** — whole-model groups at **1 and 4 threads**, closing the gap where Phase 7's headline speedup had no continuous regression guard; plus a **TSan job** at [`.github/workflows/tsan.yml`](.github/workflows/tsan.yml) — runnable, documented, but `workflow_dispatch`-only rather than gating: `-Zbuild-std` is unusable with `cargo test` ([cargo#13146](https://github.com/rust-lang/cargo/issues/13146)) and an uninstrumented `std` makes libtest itself report false races ([rust#39608](https://github.com/rust-lang/rust/issues/39608)), **resolved just after the tag**: a `--bin` harness plus an instrumented `std` gets it green, so the `CONVENTIONS.md` race-detector rule is now **satisfied** (the fix lands in v0.7.3; v0.7.2 shipped with it open) — **commit** — **PUSH + tag `v0.7.2`**

**Deliverable:** `anamnesis` v0.7.2 — GGUF-input conversions parallelised (**~1.9×** on the reader stage at the default budget, byte-identical at any thread count), a settled `RememberOptions` / `ConvertOptions` API ahead of the Python bindings, the first CI coverage of quantised-GGUF conversion, and continuous benchmarking + race detection for every threaded path. — **PUSH + tag `v0.7.2`**

**New dependencies:** None.

### Phase 7.3: Caller-Chosen Output Dtype (GGUF / `convert` path)

**Goal:** Make the dequantisation output type a **caller-chosen parameter** (`BF16`, `F32`, `F16`, and whatever comes later) instead of the hard-coded `BF16` the crate has emitted since its first commit. Ships as **`v0.7.3`** covering the `GGUF` / `convert` path, where the architecture already accommodates it; the `remember` path follows as [Phase 7.4](#phase-74-caller-chosen-output-dtype-remember-path). Both land deliberately *before* [Phase 8](#phase-8-python-bindings-pyo3).

> **⚠️ Scoped into two tags by an audit finding, not by size alone.** The original single-phase plan assumed the cheap `GGUF` path and the `TargetDtype` public API were the same piece of work. They are disjoint. [`QuantScheme`](src/parse/safetensors.rs) shows `ParsedModel::remember`, the only consumer of `TargetDtype`, covers **four kernel families across seven quantised variants** (`FP8` as `FineGrainedFp8` / `PerChannelFp8` / `PerTensorFp8`, plus `Gptq`, `Awq`, `Bnb4`, `BnbInt8`) and **never reaches a `GGUF` kernel**: `GGUF` dequant is reachable only through `convert` ([`src/convert.rs`](src/convert.rs)) and the CLI, neither of which accepts an output-dtype parameter at all today.
>
> So the phase's *motivation* (the exactness table below, measured entirely on `gguf-py`) sits on one path and the API it is named after sits on the other. **v0.7.3 takes the `GGUF` / `convert` side**: the shared writer, all 24 `GGUF` kernel functions that come with it for free, a new output-dtype parameter on `ConvertOptions`, and the direct `F32`-vs-`gguf-py` cross-validation that realises the whole exactness claim. **[Phase 7.4](#phase-74-caller-chosen-output-dtype-remember-path) takes the `remember` side**: `TargetDtype::{F32, F16}` and the four families whose kernels fuse the narrowing into the loop body and therefore cannot reuse the writer.

**Why this is a real gap, not a nice-to-have.** `TargetDtype` has had exactly one variant since commit `cdb9331` (2026-03-21) — the commit that introduced `remember`. It was marked `#[non_exhaustive]` from that day, so the design always anticipated more variants; none ever arrived. Searching the ROADMAP, CHANGELOG and every design doc turns up **no note justifying `BF16` as the only output**: the sole written rationale ([`docs/python-interop.md`](docs/python-interop.md)) argues the *binding* must not silently widen `BF16 → float32`, which presupposes the choice rather than defending it. It was **assumed, not decided.**

**Prior art: the extension points were built on day two, then never used.** This was *anticipated* even though it was never *designed*. Within 48 hours of `TargetDtype` appearing, commit `29f85b1` (2026-03-22) added two changes whose CHANGELOG entries justify them **explicitly by future variants**:

- `FromStr for TargetDtype` — *"centralizes string-to-enum parsing so new variants cannot be silently missed in the CLI"*
- `derive_output_path` matching `TargetDtype` exhaustively — *"instead of a wildcard that would silently produce broken paths for future variants"*

Together with the day-one `#[non_exhaustive]`, that is **three separate pieces of scaffolding for output dtypes that never arrived**. They are all still in place and still load-bearing for exactly one variant: `FromStr` today has a single arm (`"bf16" =>`) and an error string reading *"supported target dtypes: bf16"*. Adding variants is therefore less "add a feature" than **"hang something on the hooks the crate has carried since its second day"**.

*(One correction to that account, found while auditing this phase: `derive_output_path` no longer matches `TargetDtype` directly. [`src/cli.rs`](src/cli.rs) derives the filename suffix from `target.to_string()`, so exhaustiveness now lives one level away, in the wildcard-free `Display` impl on [`src/model.rs`](src/model.rs). The guarantee survives intact, but a new variant has to reach `Display` before the output path is right, which is a less obvious coupling than the original CHANGELOG entry describes. Both sites are listed explicitly in [Phase 7.4](#phase-74-caller-chosen-output-dtype-remember-path)'s bullets so neither is missed.)*

**Measured cost of the assumption** (`SmolLM2-135M-Q4_K_M`, gguf-py `float32` vs its round-to-nearest-even `BF16`):

| tensor | qtype | max rel err | mean rel err | exactly representable in `BF16` |
|---|---|---:|---:|---:|
| `token_embd.weight` | `Q8_0` | 3.891e-03 | 1.364e-03 | **3.31 %** |
| `blk.0.ffn_down.weight` | `Q6_K` | 3.891e-03 | 1.369e-03 | **3.29 %** |
| `blk.0.ffn_gate.weight` | `Q5_0` | 3.891e-03 | 1.308e-03 | 12.77 % |
| `blk.0.attn_k.weight` | `Q5_0` | 3.891e-03 | 1.210e-03 | 19.88 % |

**80–97 % of dequantised values are not `BF16`-representable and are rounded.** The arithmetic is forced: a `Q8_0` value is an `f16` scale (11-bit significand) × an `int8`, needing up to ~18 bits; `BF16` holds 8, `F32` holds 24. **`F32` is the only output width that does not add a rounding step of its own** (see *What "exact" does and does not mean here* below for where that stops short of mathematical exactness). The max relative error is exactly `2⁻⁸` — half a `BF16` `ULP`, as it must be.

The usual defence ("quantisation error dwarfs `BF16` rounding") holds for `Q4_K` and below but **does not hold for the high-precision types**: `Q8_0`'s own quantisation step is ~1/254 ≈ 0.39 % of the block max — the *same order* as `BF16`'s 0.39 % half-`ULP`. For `Q8_0`/`Q6_K` the crate is adding rounding comparable to the error the format exists to avoid.

This also sharpens what the crate's headline **"bit-exact, 0 `ULP`"** claim means: it is 0 `ULP` against *the reference rounded to `BF16`* — which is exactly how the fixtures are built — not against the true dequantised value.

**What "exact" does and does not mean here.** Two senses get conflated easily, and only the first is what this phase delivers:

1. **Exact against the reference.** `F32` output removes anamnesis's own narrowing step, so the emitted value *is* the `f32` that `gguf-py` (and `ggml-quants.c`) produce. This is the claim the phase makes and the claim the new cross-validation tests.
2. **Exact in the real-number sense.** True for the pure-product kernels, where every step is a multiplication of exactly-representable factors: `Q8_0` and `Q5_0` (an `f16` scale's 11 significand bits times a small int, ~18 bits) and `Q6_K` (11 + 7 + 6 = 24 bits, landing exactly on `f32`'s 24-bit significand at both of its two steps). **Not** guaranteed for the min-offset K-quants `Q2_K` / `Q4_K` / `Q5_K`, whose final `d·q - dmin·m` subtracts two `f32` values at different exponents and can itself round. There, `f32` is the reference's own value, not a mathematically exact one.

The deliverable is therefore worded as *removing a narrowing step*, not as *recovering the true value* for every kernel.

**Design: a template parameter, static inside and dynamic at the edge.** The output element type becomes a generic parameter, monomorphised exactly like a C++ template type argument, with a single runtime `match` at the public boundary so the CLI and the Phase 8 bindings (a dtype string) can still choose at run time. **The two CLI surfaces need different flag names**, because `--to` is already taken and means different things on each: `amn remember --to <dtype>` already selects an output dtype, so it simply gains `f32` / `f16` in [Phase 7.4](#phase-74-caller-chosen-output-dtype-remember-path), while `amn convert --to <format>` selects an output *format*, so the dtype arrives beside it as `--out-dtype` in this phase.

```rust
/// The element type a dequant kernel writes. Monomorphised — no dynamic
/// dispatch, no per-element branch, and the kernels never name it.
pub trait OutputElement: Copy + Send + Sync {
    const BYTES: usize;
    const DTYPE: Dtype;
    fn write_le(value: f32, out: &mut [u8]);
}
// impls: Bf16Out (default, unchanged behaviour), F32Out, F16Out
```

**The architecture already fits this on the `GGUF` side, and the cost really is far lower than it looks there.** Every `GGUF` kernel writes into an `&mut [f32]` scratch and **never mentions `BF16`**; the narrowing happens at exactly **two call sites**, inside `run_legacy_kernel` and `run_super_kernel`, both calling the single `write_scratch_to_bf16` ([`src/remember/gguf.rs`](src/remember/gguf.rs)). Making that tiny writer generic (rather than the kernels) leaves **all 24 kernel functions untouched** (the 22 production block-quant kernels, plus the activation-only `Q8_1` / `Q8_K` that ship unit-tested but uncross-validated) and bounds monomorphisation to one small function per output type instead of 24 kernels × N types.

**It does not fit the other four families, and that is the whole reason for the split.** `f32_bits_to_bf16_bits` ([`src/remember/fp8.rs`](src/remember/fp8.rs)) is a shared *helper*, not a shared *writer*: `FP8`, `BnB`, `GPTQ`, and `AWQ` each call it from inside their own kernel body, with the `× 2` output-offset arithmetic woven through the loop nest (the `FP8` block path computes it at row, block, and remainder level). "Kernels untouched" is achievable for `GGUF` and impossible for those four, so they get their own tag with their own bullets. See [Phase 7.4](#phase-74-caller-chosen-output-dtype-remember-path).

**Two things this unlocks beyond precision:**

1. **A strictly stronger correctness claim**, in sense 1 above: the cross-validation stops rounding gguf-py's `float32` down to `BF16` before comparing, so it tests the reference as the reference actually produces it.
2. **It de-risks Phase 8.** `NumPy` has native `float32` and `float16` but **no** `bfloat16`; the frozen `ml_dtypes` return contract exists solely to work around a limitation that an `F32`/`F16` output removes. A Python user who wants a plain `np.float32` array — i.e. most of them — would get one with no optional dependency. That payoff completes at [Phase 7.4](#phase-74-caller-chosen-output-dtype-remember-path), since `remember` is the entry point the bindings expose; v0.7.3 delivers it for `convert` and establishes the trait the bindings will inherit.

- [x] **Dependency refresh + the two deliberate pins documented** — `cargo update` moved 52 packages to their latest semver-compatible versions, all transitive. `criterion` verified to stay major-5 (CodSpeed *walltime* fails to collect across a major mismatch) and `zip` held at v2 with v8 available, because it is the dev-only differential oracle the vendored `src/parse/zip.rs` is checked against; both rationales now sit in `Cargo.toml` beside the pins, since `cargo update` will keep reporting `zip` v8 forever — **commit**
- [x] **Rust Edition 2024** — `rust-version = "1.88"` already cleared the 1.85 floor, so the MSRV is unchanged. **One site changed meaning**: `src/parallel.rs`'s `handle.join()` was a `match` scrutinee whose temporary may carry a custom destructor, and 2024 drops it earlier (`tail_expr_drop_order`); binding it to a local pins the drop point under both editions, and the reordering was *checked* to be inert (no lock released, no message sent) rather than assumed. The rest is spelling: eight let-chain collapses now that they are stable, and `unsafe_op_in_unsafe_fn` made per-operation in the AVX2 experiment. Cargo's resolver moves to **v3** (MSRV-aware); it selected nothing new, so `Cargo.lock` is byte-identical across the flip. The rustfmt 2024 style edition lands as its **own** commit so the migration diff stays readable — **2 commits**
- [x] **The published crate drops the test corpus: 4.8 MiB → 0.60 MiB (8.0×)** — `tests/` joins `fuzz/` in `Cargo.toml`'s `exclude`. `tests/fixtures` alone was 6.33 MiB, 71 % of everything tracked and reachable from no consumer's build. The real motivation is not the download: the 10 MiB registry cap had begun to bound how wide the cross-validation goldens could be, which is backwards. A test corpus should be sized by the correctness claim it supports, the artefact by what consumers need. The corpus now ships via a **GitHub Release per tag** (the repo had none before), created by `publish.yml` *after* `cargo publish` succeeds, plus `scripts/verify-claims.{sh,ps1}` — **2 commits**
- [x] **`OutputElement` trait + generic `write_scratch`** — `Bf16Out` / `F32Out` / `F16Out`, replacing `write_scratch_to_bf16`; all 24 `GGUF` kernel functions untouched; `dequantize_gguf_to_bf16` / `dequantize_gguf_blocks_to_bf16` retained as thin `::<Bf16Out>` wrappers so nothing breaks — **commit**
- [x] **Generalise the fixed-width arithmetic the `GGUF` path hard-codes** — `validate_dequant_input`'s output length and 32-bit overflow guard are `n_elements × 2` and become `n_elements × E::BYTES`; both kernel runners size their block buffers from `E::BYTES` rather than a literal `QK × 2` — **commit**
- [x] **The streaming sink contract becomes dtype-dependent** — `dequantize_gguf_blocks_to_bf16` documents its sink as receiving 64 B per legacy block and 512 B per K-quant super-block, and those become `QK × E::BYTES`; public doc updated plus a test asserting the observed block length per output type, so a caller's `chunks_exact(2)` assumption fails loudly rather than silently truncating. *(Gap the original single-phase plan did not name.)* — **commit**
- [x] **Output dtype on the `convert` path, the parameter that does not exist yet** — `ConvertOptions::output_dtype` / `with_output_dtype` as a builder (matching `with_threads`), threaded through `read_gguf` / `read_hub` and both writers, replacing the hard-coded `Dtype::BF16` in [`src/convert.rs`](src/convert.rs) and [`src/cli.rs`](src/cli.rs); CLI `--out-dtype bf16|f32|f16` on `convert`, named that way because `convert --to` already means output *format*. Without this the new generic writer has **no caller**, since `TargetDtype` does not reach the `GGUF` path — **commit**
- [x] **Passthrough policy, stated rather than assumed** — `remember` and `convert` already emit mixed-dtype files, with dequantised tensors at `BF16` and passthrough tensors keeping their source dtype ([`src/model.rs`](src/model.rs)); decide and document that an `F32` request widens **dequantised tensors only** and leaves passthrough untouched, so nobody reads `--out-dtype f32` as "rewrite every tensor as `F32`" — **commit**
- [x] **~~`inspect`'s dequantised-size estimate made dtype-aware~~ — moved to [Phase 7.4](#phase-74-caller-chosen-output-dtype-remember-path).** Found while implementing: [`src/inspect.rs`](src/inspect.rs) is `From<&SafetensorsHeader>` only and has **no `GGUF` path at all**, so the `num_elements × 2` estimate it carries governs the `remember` side. No `--out-dtype` can reach it under this phase's scope, and it becomes wrong only when 7.4 makes the `remember` output width variable — **re-scoped, not skipped**
- [x] **`F16` range policy plus tests, not just an accuracy number** — unlike `BF16`, `F16` does **not** share `f32`'s exponent range: `e8m0_to_fp32_half` spans `2⁻¹²⁸` to `2¹²⁷`, while `F16` saturates at 65504 and flushes to zero below roughly `2⁻²⁴`. Define and test the overflow / underflow behaviour (saturate, or refuse) before a real `MXFP4` or `IQ` file discovers it. `F16` is **not** uniformly the better 2-byte choice: it buys 3 significand bits and pays a far narrower exponent range — **commit**
- [x] **Cross-validation at `F32`, compared directly against `gguf-py` with no rounding step** — extend `tests/fixtures/gguf_reference/generate_gguf.py` to store `float32` goldens; it already asserts `f32.dtype == np.float32` before rounding, and the fixtures already carry the raw quantised bytes, so this needs no model re-download. **Budget for this failing, and do not treat it as a formality.** Every existing `GGUF` cross-validation rounds to `BF16` first and so discards 16 mantissa bits: no kernel's `f32` has ever been compared against the reference at full width. `Q4_K` and `Q6_K` were spot-checked during planning and associate identically to `gguf-py` (`(d·sc)·q`, and `d·sc·q - dmin·m`), but the remaining 22 of the 24 have not been audited (a different 22 from the production count above, which happens to coincide), and a stray float contraction on a `d·q - dm` line would be equally invisible today. Audit association order across all 24 as part of this step — **commit**
- [x] **`tests/peak_heap_gguf.rs`** — `dhat` peak-heap tests exist for `AWQ`, `GPTQ`, and `BnB` but **not** for `GGUF`, which is precisely the path whose output width is about to become variable; add one, parameterised over the output type, so the `# Memory` claims stay verified per dtype rather than per assumption — **commit**
- [x] **CodSpeed: guard the `BF16` path, record the `F32` cost** — the existing groups already guard the default path against a monomorphisation regression, and they do so precisely: `dequant_gguf_q4_k` drives `run_super_kernel` and `dequant_gguf_q8_0_ollama` drives `run_legacy_kernel`, the two call sites this phase makes generic, so **no new benchmark is needed for the regression guard**. What is missing is the cost curve for the new option: add an `F32` bench id inside `dequant_gguf_q4_k` (kernel level) and a sibling `convert_gguf_to_safetensors` id (end to end, where the doubled write meets the output stage Experiment 12 measured at ~60 % of wall clock). **No `F16` arm** — same width as `BF16`, so no bandwidth story, and its interesting properties (accuracy, exponent range) are tests rather than benchmarks. These arms **record rather than gate**, consistent with the capability-not-perf-claim framing below; the workflow runs `mode: walltime` on macro runners, so they report real memory-bandwidth cost instead of an instruction count that would hide it — **commit**
- [x] **Measure and record, but this is a capability, not a perf claim** — `F32` doubles output bytes on a path Experiment 12 showed is bandwidth-bound, so expect it to be **slower** than `BF16` and expect the Phase 7.2 threading ratios to shift. **That is not a blocker.** The [`CLAUDE.md`](CLAUDE.md) "no measured win → no commit" rule governs *perf-claim* commits; this phase claims **exactness**, not speed, and a correct-but-slower `F32` path is the point of it. Record the numbers in `docs/perf-experiments.md` as the honest cost of the new option, then optimise it as its own follow-up rather than gating the capability on it. Also verify binary-size growth from monomorphisation, and quantify `F16`'s accuracy gain (11-bit significand vs `BF16`'s 8) against the range cost measured above — **commit** — **PUSH + tag `v0.7.3`**

**Deliverable:** `anamnesis` v0.7.3 — the `GGUF` / `convert` output dtype is a caller-chosen parameter, and `F32` output drops anamnesis's own narrowing step so the result is the reference `f32` itself, validated against `gguf-py` with no rounding in between for the first time. — **PUSH + tag `v0.7.3`**

**Outcome (shipped as `v0.7.3`).** Every bullet above is done; the `inspect` one was re-scoped to [Phase 7.4](#phase-74-caller-chosen-output-dtype-remember-path) with its reason recorded in place. Four results worth carrying forward:

1. **The `F32` cross-validation passed 22/22 on the first run**, exactly, with no tolerance. This phase's plan said to budget for it failing and not to treat it as a formality, so the null result is worth stating: all 22 production kernels already associate their arithmetic identically to `gguf-py`. Nothing needed fixing; what changed is that it is now *verified* rather than assumed, and that settles the association-order audit this phase asked for. The comparison was shown to have teeth rather than assumed to: **76.5 %** of the 1 441 792 reference values carry mantissa bits `BF16` cannot hold, and a one-bit mutation in a single golden fails exactly one element at 1 `ULP` while the `BF16` comparison stays green.
2. **Peak heap equals output exactly, at every width** (`0 B` of overhead, `dhat`, 45 M-element `Q4_K`), and the streaming entry point peaks at **`0 B`**. The `# Memory` claims are now confirmed literally rather than argued.
3. **`F32` costs 1.79× at the kernel and 1.54–1.61× end to end** against 2.00× of output bytes, so the cost is the doubled write and essentially nothing else. The Phase 7.2 threading ratio shifts 1.19× → 1.14× as a more bandwidth-bound path should; **do not quote Phase 7.2's ratios for `F32`**. The `BF16` default did not regress (`p = 0.41`, `p = 0.13`). See `docs/perf-experiments.md` Experiment 13.
4. **All three writers vectorise** (`Bf16Out` 8-wide AVX2, `F32Out` 8-wide stores, `F16Out` 4-wide `F16C`), which is the payoff for putting the loop on the trait rather than a per-element hook: three independent loops, three independent verdicts.

One correction to this phase's own text, for the next reader: "all 24 `GGUF` kernel functions untouched" is true of the kernel **bodies** (the bit manipulation, formulas and annotations are byte-identical) but not of their **signatures**, which thread `<E>` through. The economy the phase was built on holds; the wording overstated it slightly.

**New dependencies:** None (`half` already provides `f16`).

### Phase 7.4: Caller-Chosen Output Dtype (`remember` path)

**Goal:** Finish what [Phase 7.3](#phase-73-caller-chosen-output-dtype-gguf--convert-path) starts by extending the caller-chosen output dtype to `ParsedModel::remember`, which means filling in `TargetDtype::{F32, F16}` and making the four kernel families `remember` dispatches over (`FP8`, `GPTQ`, `AWQ`, `BnB`, spanning the seven quantised `QuantScheme` variants) generic over `OutputElement`. Ships as **`v0.7.4`**, still ahead of [Phase 8](#phase-8-python-bindings-pyo3), so the Python bindings freeze a settled dtype contract on **both** paths rather than a `BF16`-only one.

**Why this is the larger half.** Phase 7.3's cost is low because `GGUF` funnels all 24 of its kernel functions through one shared writer. These four families have no such funnel: each calls `f32_bits_to_bf16_bits` from inside its own hot loop, and each computes output offsets in units of 2 bytes at every level of its loop nest. Outside `GGUF` there are **five call sites of that helper, sitting inside eight distinct hot loops** (`FP8` reaches its through `e4m3_to_scaled_bf16` from four of them), so generalising means editing kernel bodies rather than a single helper. The `f32` values still exist and are still computed the same way, so the *idea* carries over unchanged; only the "one small function" economy does not.

- [x] **`TargetDtype::{F32, F16}` + the boundary dispatch** — the three `match target` sites in [`src/model.rs`](src/model.rs) (`remember_with_options`, the progress variant, and `remember_to_bytes`), plus the `FromStr` single `"bf16"` arm, its *"supported target dtypes: bf16"* error string, and the `Display` impl that [`src/cli.rs`](src/cli.rs)'s `derive_output_path` now depends on for its filename suffix — **commit**
- [x] **The four fused-narrowing families made generic over `OutputElement`** — `FP8` (`e4m3_to_scaled_bf16` and its four call sites, plus the row / block / remainder offset arithmetic), `BnB` (the `NF4`/`FP4` pass 2 and the `INT8` loop), `GPTQ`, and `AWQ`; existing `dequantize_*_to_bf16` entry points retained as `::<Bf16Out>` wrappers. **This is the bulk of the work across the 7.3 / 7.4 pair** — **commit**
- [x] **Output-side plumbing** — `build_views` hard-codes `safetensors::Dtype::BF16` for every dequantised tensor ([`src/model.rs`](src/model.rs)); CLI `--to f32` / `--to f16` on `remember`, which needs no new flag because `remember --to` already selects a dtype; plus the same passthrough policy Phase 7.3 documents for `convert`, applied and stated here — **commit**
- [x] **`# Memory` doc sections and `dhat` assertions made dtype-parameterised** — every one currently reads `2 × n_parameters` or, in code, `total_elements * 2`: three `# Memory` sections on `remember`, plus `tests/peak_heap_awq.rs`, `tests/peak_heap_gptq.rs`, and `tests/peak_heap_bnb_dq.rs` at two assertions each. [`src/lib.rs`](src/lib.rs) promises these are verified to the byte on the reference, so they have to be re-derived per output type, not merely reworded — **commit**
- [x] **Cross-validation at `F32` for all four families** — against the same Python references the `BF16` suites already use, with no rounding step in between; same caveat as Phase 7.3, in that rounding to `BF16` has been hiding 16 mantissa bits of any disagreement, so treat first-run failures as real findings rather than test bugs — **commit**
- [x] **CodSpeed: the guard is already in place, add one recording arm** — `benches/dequant.rs` already covers all four families (`FP8` per-tensor and fine-grained, `GPTQ`, `AWQ`, `BnB` `NF4` and `INT8`), so the `BF16` regression guard exists **before** these kernels are touched, which is the order that matters. Add a single sibling group `remember_f32_whole_model` in `benches/convert.rs` alongside the existing `remember_bf16_whole_model`, at the same 1-and-4-thread budgets. **Add rather than rename**: renaming the existing group would orphan its CodSpeed history, and that `BF16` series is exactly the baseline this phase must not regress. No `F16` arm, and these arms record rather than gate, per Phase 7.3 — **commit**
- [x] **`docs/python-interop.md` restated** — the frozen `BF16` return contract predates a caller-chosen dtype, and its argument (the binding must not silently widen `BF16` to `float32`) presupposes the choice rather than defending it. Restate it precisely: the binding still never widens, but the caller can now *ask* for `F32` or `F16` up front, which is what most `NumPy` users want and what removes the optional `ml_dtypes` dependency from the common path. This is the concrete Phase 8 de-risking the pair buys — **commit**
- [x] **Small adjacent fix, same surface** — `to_bf16_bytes` in [`src/convert.rs`](src/convert.rs) (the `bnb`-gated encode-side narrowing helper) **truncates** with `bits >> 16` while its own doc comment claims it matches the crate's `f32_bits_to_bf16_bits` convention, which rounds to nearest even; one of the two is wrong, so decide which and align them while the narrowing surface is already under the microscope — **commit**
- [x] **`inspect`'s dequantised-size estimate made dtype-aware** — *(moved here from [Phase 7.3](#phase-73-caller-chosen-output-dtype-gguf--convert-path) during implementation.)* [`src/inspect.rs`](src/inspect.rs) estimates output at `num_elements × 2` (and `byte_len × 4` for `BnB4`), and that figure feeds the [Phase 6.8](#phase-68-untrusted-input-resource-governance) inspect-before-parse policy gate. It belongs **here** rather than in 7.3 because `InspectInfo` is `From<&SafetensorsHeader>` only and has no `GGUF` path, so the estimate governs the `remember` side exclusively: it becomes wrong at the moment *this* phase makes that width variable, not before. Under-reporting by 2× in the one place whose entire job is telling a caller how big the result will be — **commit**
- [x] **Measure and record** — on the same capability-not-perf-claim footing as Phase 7.3: the `F32` and `F16` cost for each of the four families in `docs/perf-experiments.md`, plus the monomorphisation binary-size delta now that four kernel families rather than one writer are duplicated per output type. A slower `F32` path is expected and is not a blocker — **commit** — **PUSH + tag `v0.7.4`**

**Deliverable:** `anamnesis` v0.7.4 — the output dtype is a caller-chosen parameter on **every** path the crate exposes, `remember` included, and [Phase 8](#phase-8-python-bindings-pyo3) inherits a settled dtype contract where a Python caller asking for a plain `np.float32` array gets one with no optional dependency. — **PUSH + tag `v0.7.4`**

**Outcome (shipped as `v0.7.4`).** Every bullet above is done. Unlike Phase 7.3, whose `F32` cross-validation passed 22/22 on the first run, this phase's did **not**, and the failure was the most valuable thing in it. Six results worth carrying forward:

1. **The `F32` cross-validation found a real kernel defect, in `BnB` `INT8`.** anamnesis computed `w × (SCB / 127)`; `bitsandbytes` computes `(w × SCB) × (1/127)`. Same real number, same `BF16`, but **17610/65536 (26.9 %)** of elements differ by exactly 1 `ULP` at `F32`. Five releases of `BF16` cross-validation had reported 0 mismatches, because the divergence lives entirely in bits `BF16` discards. The constant is not the variable: `f32(7.874015718698502e-3)` and `f32(1.0/127.0)` are the same bits, now asserted at compile time. Fixed by matching the canonical association; **no `BF16` output byte changed**. This is the phase's headline, and it validates the plan's own instruction to budget for failure rather than treat the comparison as a formality.
2. **The other three families passed at `F32` on the first run** (`FP8` 7/7, `GPTQ` 4/4, `AWQ` 2/2, plus `BnB`'s six 4-bit fixtures). The comparison was shown to have teeth rather than assumed to: **38–98 %** of values across the fixtures carry mantissa bits `BF16` cannot hold, the low end being the `BnB` `FP4` pair whose 16-entry codebook lands on representable values more often.
3. **~~`MAX_OUTPUT_BYTES` widening~~ was not needed, and v0.7.3's prediction was wrong.** Phase 7.3 left a note in [`src/remember/output.rs`](src/remember/output.rs) expecting v0.7.4 to widen that `gguf`-gated constant once the four families went generic. It did not, and the gate is correct as it stands: these four families tile through an **`f32` scratch** and hand it to `OutputElement::write_scratch`, so their scratch is sized in `f32`s and never in output bytes. Only the `GGUF` runners build a byte tile up front, because their kernels write into a fixed `[f32; QK]` beside a materialised output buffer. `MAX_OUTPUT_BYTES` stays a `GGUF` concept, and the note now says so at the constant.
4. **`write_one` was designed, implemented, measured, and rejected on asm evidence.** A per-element trait hook would have been the obvious way to generalise a fused loop, but it de-vectorised `GPTQ` by **4.25×** and `FP8` by **1.46×** to scalar. `write_scratch` (a loop on the trait, not an element hook) is what ships, and the rejection is recorded at the trait so it is not re-proposed.
5. **The perf investigation surfaced a downstream bug worth more than the perf question.** Two benches disagreed by ~1.9× about whether the split was faster or slower; the tell was that the end-to-end path measured *faster per element* than a kernel it contains. Root cause: making `dequantize_*_to_bf16` an `#[inline]` generic wrapper let an external crate instantiate the generic locally, where four private helpers were opaque, so **any downstream caller was ~2.9× slower than v0.7.3**. Fixed by `#[inline]` on those helpers. Post-fix the split is a net win: `FP8` **2.2× faster** at the kernel and 1.8× whole-model, the other three paying an honest 1.06–1.15×. See `docs/perf-experiments.md` Experiments 14 and 15.
6. **A determinism test that had never entered the code it named.** `MIN_PARALLEL_BYTES` is 4 `MiB` of input and the `remember` determinism fixture was 32 bytes, so every thread-count assertion had been running the sequential path since Phase 7. The per-dtype test added here uses a fixture sized off the constant, and `remember_fixture_crosses_the_parallel_threshold` now asserts the gate is cleared rather than hoped for. `CONVENTIONS.md` § *Verify parallelism* point 5 made executable.
...
**Outcome (shipped as `v0.7.5`).** Every bullet above is done and
...
path — was also corrected. Shipped as **v0.7.5** on
2026-08-18 (`31cb17e`), tagged and pushed.

**New dependencies:** None.

### Phase 7.6: API parity before the bindings

**Goal:** Give every verb the crate exposes **one shape across all four formats**, before [Phase 8](#phase-8-python-bindings-pyo3) freezes those shapes into a PyPI signature. Ships as **`v0.7.6`**, the last *API-shaping* tag of the 0.7 line ([Phase 7.7](#phase-77-settle-each-suppressed-site-on-its-own-measurement) follows it with kernel-internal work that changes no signature). Nothing here is a new capability: every item is an asymmetry the API grew while each format was added one phase at a time, plus one measured defect that ships today.

**Provenance.** Audited 2026-08-20 against `main` @ `31cb17e` (v0.7.5, tagged and pushed). This phase is the **complement** of [`docs/EXTERNAL AUDIT/2026-08-17 AMN v0-7-4 PHASE-8-READINESS AUDIT AC OPUS.md`](docs/EXTERNAL%20AUDIT/2026-08-17%20AMN%20v0-7-4%20PHASE-8-READINESS%20AUDIT%20AC%20OPUS.md), not a re-run of it. That audit checked the **hardening** prerequisites row by row (panic discipline, the `ParseLimits` choke point, pickle-VM governance, the error taxonomy, `Send + Sync`) and found them green; its single finding, F-1, is fixed in `0d84b7b`. What it did not examine is the axis this phase covers: **the shape of the public API itself** — which verbs exist for which formats, which types the crate hands back, and which of those can still grow a field. Re-verified green at the same commit before this phase was written: `cargo test --all-features` (exit 0), tag `v0.7.5` present on `origin`, and [Experiment 12](docs/perf-experiments.md)'s CI-contradicted threading figure now qualified.

**Why its own tag rather than a Phase 8 step.** Phase 8's own text argues the case: *"PyPI punishes signature churn far more than crates.io does"*, and its encode-surface bullet rejects option (a) precisely because it would mean **inventing Rust API under Python deadline pressure**. Every item below is Rust API. Doing them inside Phase 8 reproduces the failure mode that bullet was written to avoid, and it makes the first PyPI signature a transcription of four different per-format conventions. One phase, one tag, and Phase 8 then starts on Step 0 (the pitch) against a settled surface.

**Items 1–5 either ship broken today or become permanent the moment a wheel is published. Items 6–10 are cheaper and reversible, but every one of them is a first-contact experience for a `pip install` audience.**

- [x] **1. `GGUF` `remember` exists only inside the CLI, and it is measurably slower than the library path that produces the same bytes.** [`src/cli.rs`](src/cli.rs) `run_remember_gguf` (lines 634–754, **121 lines**) re-implements shape reversal, dtype mapping, dequant dispatch, `TensorView` assembly and `serialize_to_file`. [`src/convert.rs`](src/convert.rs) `read_gguf_as` (lines 834–912) does the same work **threaded** through `parallel::map_indexed`, generic over `OutputElement`, and under caller `ParseLimits`. The CLI copy is sequential and **never receives `--threads`**: the dispatch in `run_remember` calls `run_remember_gguf(path, output, target)` with no threads parameter, so `amn remember model.gguf --threads 8` is silently ignored. Measured on the audit machine, release build (`cargo build --release --features cli,gguf`, no `target-cpu=native`), wall clock via `Measure-Command`, same input and same output for both verbs:

  | Fixture | `amn remember --to bf16` | `amn convert --to safetensors --out-dtype bf16` | Ratio |
  |---|---|---|---|
  | `SmolLM2-135M-Instruct-Q6_K.gguf` (132 MiB), best of 3 | **237 ms** (237 / 245 / 314) | **171 ms** (171 / 176 / 199) | **1.39×** |
  | `Qwen2.5-1.5B-Instruct-IQ2_M.gguf` (574 MiB), best of 2 | **6032 ms** (6032 / 6201) | **2562 ms** (2562 / 2642) | **2.35×** |

  The two outputs are **`SHA-256` identical** (269 125 560 bytes for the SmolLM2 pair), which is what makes this a duplication rather than a difference: one operation, two implementations, and the flagship verb dispatches to the slower one. *Method caveat, stated so the fix commit does not inherit it:* the table is best-of-3 / best-of-2 wall clock taken to size the defect, **not** the best-of-5 `target-cpu=native` median [`CLAUDE.md`](CLAUDE.md) requires of a perf-claim commit; the fix ships under that rule with its own before/after numbers. **The Phase 8 consequence is sharper than the speed:** there is no library entry point for a `GGUF` `remember()` binding to call, so the binding would be the *third* implementation of this loop. Fix by moving the operation into the library (`ParsedGguf::remember` / `remember_with_options`, the `ParsedModel::remember` spelling) and routing the CLI at it, with an equivalence test pinning the byte-identical result above. **Peak heap is not part of the charge, and the phrasing matters**: this path collects a `Vec<(String, Vec<u8>, Vec<usize>, Dtype)>` over every tensor because `serialize_to_file` needs all views alive at once, but so does `ParsedModel::remember` (its own `# Memory` section puts the dequantised set at `O(target.byte_size() × n_parameters)`, streaming only the *write*), and so does `convert`'s hub. All three are whole-model resident, so routing the CLI at the library changes speed and thread control, not memory. Bounding that is [Phase 10](#phase-10-streaming-output)'s job, and this item only forbids doc comments that imply it is already done — **commit**
- [x] **2. A dequantised-size estimate exists for exactly one format, and `inspect` has no common shape across the four.** [`src/inspect.rs`](src/inspect.rs) `InspectInfo` (line 77) carries a dtype-aware `dequantized_size` **and** the `output_dtype` it assumed. Its three siblings carry neither, and share no trait: `GgufInspectInfo` ([`src/parse/gguf.rs`](src/parse/gguf.rs) line 952: `version`, `architecture`, `tensor_count`, `total_bytes`, `unknown_size_tensors`, `dtypes`, `alignment`), `PthInspectInfo` ([`src/parse/pth.rs`](src/parse/pth.rs) line 619: `tensor_count`, `total_bytes`, `dtypes`, `big_endian`), and `NpzInspectInfo` ([`src/parse/npz.rs`](src/parse/npz.rs) line 627: `tensors`, `total_bytes`, `dtypes`). **`GGUF` is where the omission bites hardest**, because it is the only format whose on-disk size does not predict its dequantised size: the ratio is per-kernel, so a `Q2_K` tensor and a `Q6_K` tensor of equal length expand by different multiples. That is precisely the number the [Phase 6.8](#phase-68-untrusted-input-resource-governance) inspect-before-parse gate exists to check, and the one a caller cannot derive. Phase 8 promises a binding where *"`inspect()` takes the **output dtype** and returns the estimate for *that* width"* and where a figure *"can never be read without its width"*; today that holds for one format in four, and Python needs a single return type. Two consequences in the same place, both fixed here: **(a)** `amn inspect` takes no dtype flag ([`src/cli.rs`](src/cli.rs) line 39, `Inspect { path }`), so the CLI's own gate is pinned to the `BF16` assumption, which is the identical bug class v0.7.4 fixed for `remember` and did not carry across, while the subcommand's help text already advertises *"size estimates"*; **(b)** [`docs/tutorials/inspect-before-you-parse.md`](docs/tutorials/inspect-before-you-parse.md) line 69 promises the reader a size *"both on disk and what it will become if you dequantize"*, then illustrates it with a `GGUF` for which only the on-disk figure exists. Decide the unified shape (a shared trait, or one `InspectInfo` with a format-tagged variant) here rather than letting the binding decide it by accident — **commit**
- [x] **3. `NPZ` has no in-memory or reader-based parse.** safetensors, `.pth` and `GGUF` each expose `parse_X_bytes`, `parse_X_from_reader`, and a `_with_limits` variant of both. `NPZ` exposes `parse_npz(path)` and `parse_npz_with_limits(path, limits)`, and nothing else. So [Phase 6.13](#phase-613-python-readiness-hardening)'s copy-based untrusted-input contract has **no `NPZ` instance**, and Phase 8's hardening step, *"route the binding's untrusted entry through 6.13's copy-based `parse_*_from_reader`"*, cannot be honoured for the format named in its **first** binding bullet (`parse_npz()`). The gap is ergonomic as well as architectural: a Python caller holds bytes (`io.BytesIO`, an HTTP response, a `datasets` blob) far more often than a path, and today has to write a temp file. Add `parse_npz_bytes` / `parse_npz_from_reader` plus their `_with_limits` forms, mirroring [`src/model.rs`](src/model.rs) lines 276–351; the machinery exists already, since `inspect_npz_from_reader` proves the container reader is reader-generic. **The same hole on the other verb, decided here rather than in glue:** `convert` is `Path` → `Path` only ([`src/convert.rs`](src/convert.rs) line 571), while the bytes-out family exists piecemeal elsewhere (`remember_to_bytes`, `pth_to_safetensors_bytes`, `npz_to_safetensors_bytes`, `write_bnb_nf4_safetensors_bytes`, `write_gguf_to_writer`). Either give `convert` an in-memory form or state on the record that conversion is a file-to-file operation, because a notebook caller will ask — **commit**
- [x] **4. The report structs the crate hands back are not `#[non_exhaustive]`, so item 2 cannot be fixed additively.** Inventory at `31cb17e`: **all 11 public enums** carry the attribute (`AnamnesisError`, `Dtype`, `QuantScheme`, `TensorRole`, `TargetDtype`, `ConvertTarget`, `GgufType`, `GgufMetadataValue`, `GgufMetadataArray`, `NpzDtype`, `PthDtype`), and so do the six option / stat types (`ParseLimits`, `InspectOptions`, `InspectInfo`, `RememberOptions`, `ConvertOptions`, `ConvertStats`). **28 public structs do not**; **22 of those have all-public fields**; and **20 of those 22 are values the crate returns**: `TensorEntry`, `SafetensorsHeader`, `GptqConfig`, `AwqConfig`, `BnbConfig`, `GptqCompanions`, `AwqCompanions`, `Bnb4Companions`, `GgufTensorInfo`, `GgufTensor`, `GgufInspectInfo`, `GgufFrontMatter`, `NpzTensor`, `NpzTensorInfo`, `NpzInspectInfo`, `PthTensor`, `PthTensorInfo`, `PthInspectInfo`, `PthFrontMatter`, `BnbNf4WriteStats`. `InspectInfo` was marked at v0.7.4 with the comment *"`#[non_exhaustive]` since v0.7.4: the Python bindings freeze this shape in Phase 8, and a struct whose fields are all `pub` cannot otherwise gain one without a breaking change"*: the reasoning is exactly this item's, and it was applied to exactly one struct. Adding `dequantized_size` to `GgufInspectInfo` (item 2) is a breaking change unless the attribute lands first, so **this item gates that one**. **Two exclusions, deliberately.** `BnbWriteInput` and `GgufWriteTensor` are *caller-constructed inputs*, where `#[non_exhaustive]` would break the callers who have to build them: leave them, or give them constructors first. `ParsedModel` (one private field) and `ParsedGguf` / `ParsedPth` (all fields private) are already unconstructible from outside, so the attribute would only forbid exhaustive destructuring; low value, but decide it explicitly rather than by omission — **commit**
- [x] **5. Nothing in the API can be cancelled, and `convert` cannot even report progress.** The progress hook is `F: FnMut()` returning `()` ([`src/model.rs`](src/model.rs) lines 750 and 783): it can count tensors and cannot say *stop*. `convert()` has no hook at all. Phase 8's hardening step commits to **releasing the GIL** during large `parse` / `remember` / `convert`, and a released GIL means `KeyboardInterrupt` is not delivered until Rust returns, so a notebook user cannot `Ctrl-C` a multi-minute conversion and a web worker cannot honour a request timeout. Neither the roadmap nor the external audit raises this. It is Rust API, and the callback shape is already public, so settle it here: give the callback a return type that can request cancellation (`ControlFlow`, or `crate::Result<()>` so the reason surfaces as an ordinary `Err`), and give `convert` the same hook. Cancellation must not leave a half-written output file behind silently: decide and document whether the partial file is removed or left to the caller — **commit**
- [x] **6. Format detection is `pub(crate)`, and `parse()` means something different in the library than on the command line.** `Format` and `detect_format` live at [`src/convert.rs`](src/convert.rs) lines 294 and 351, both `pub(crate)`. A polymorphic Python `inspect(path)` or `parse(path)`, which is what `amn` already offers and what a Python caller will assume, has nothing to call, so the binding would re-sniff by extension inside PyO3 glue. Promote both, or expose a `detect_format` returning a public enum. **The naming trap is the sharper half:** `anamnesis::parse()` is **safetensors-only** while `amn parse` dispatches over four formats ([`src/cli.rs`](src/cli.rs) line 241, `run_parse`). Binding `parse()` to the Rust function would give Python a function that rejects three of the four supported formats under the crate's most obvious name, permanently. Decide the Python spelling here, in Rust, and record it — **commit**
- [x] **7. No `inspect` entry point accepts `ParseLimits`, while the README calls the recommended one "bounded".** All three summary readers are limit-free by identical design comment: `inspect_npz_from_reader` ([`src/parse/npz.rs`](src/parse/npz.rs) line 773, passing `ParseLimits::unbounded()` at line 778), `inspect_gguf_from_reader` ([`src/parse/gguf.rs`](src/parse/gguf.rs) line 2308), and `inspect_pth_from_reader` ([`src/parse/pth.rs`](src/parse/pth.rs) line 2644). The **decision** is defensible: report the totals, let the host apply its own policy, then parse under limits. The **wording** is not. [`README.md`](README.md) lines 129–132 present exactly this call as *"Inspect cheaply: header-only, **bounded**"* inside the flagship untrusted-input recipe. It is bounded only by the permanent floors, and for `NPZ` that floor is `ZIP_MAX_ENTRIES = 1 << 20` ([`src/parse/zip.rs`](src/parse/zip.rs) line 103), so a hostile archive can force a million-entry central-directory walk, a per-entry `NPY` header read bounded only by `NPY_MAX_HEADER_BYTES = 1 << 20` ([`src/parse/npz.rs`](src/parse/npz.rs) line 48), and a `Vec<NpzTensorInfo>` the caller cannot cap, on the call the documentation tells a multi-tenant host to make **first**. That inverts [Phase 6.8](#phase-68-untrusted-input-resource-governance)'s own thesis, which is that a caller can always tighten. `GGUF` and `.pth` at least have a bounded alternative in `parse_*_front_matter_from_reader_with_limits`; `NPZ` has none. Either add `_with_limits` variants to the three inspects (preferred: it is the same one-parameter change [Phase 7.5](#phase-75-pth-reader-generic-front-matter) already made to `read_pth_archive_for_inspect`), or correct the README's word and name the floors that do apply. Phase 8's *"surface `ParseLimits` to Python callers"* needs the first answer — **commit**
- [x] **8. `NPZ` rejects Fortran-order arrays, which `NumPy` itself writes.** [`src/parse/npz.rs`](src/parse/npz.rs) lines 804 and 944 return `Unsupported` when the `NPY` header says `fortran_order: True`. `np.savez("w.npz", w=x.T)` writes exactly that, because a transposed 2-D view is `F`-contiguous and `numpy.lib.format` records the order it finds. Harmless while the audience was Rust consumers of `C`-order SAE archives; a first-contact failure for a `pip install anamnesis-quant` user on a file `NumPy` wrote unprompted. Support it (a read-time transposition for rank 2, with an explicit `Unsupported` for higher ranks if the general case is not worth it yet) or decide against it on the record, but decide before the wheel rather than after the first issue — **commit**
- [x] **9. `amn remember file.npz` errors on an operation the library performs.** [`src/cli.rs`](src/cli.rs) line 459 rejects `NPZ` with *"NPZ tensors are already full-precision; no dequantization or conversion needed"*, while `npz_to_safetensors` is exported and `amn convert file.npz --to safetensors` does the conversion. The `.pth` arm of the same `match` resolves the identical tension the opposite way, accepting the dtype as vacuous because refusing it would be *"hostile"*. A Python `remember()` has to pick one of these behaviours for all formats: pick it here, and make the CLI and the library agree — **commit**
- [x] **10. Doc rot that reaches users.** Phase 7.5 shipped as PTH reader-generic front matter, but **15 occurrences across 8 source files** ([`src/cli.rs`](src/cli.rs), [`src/lib.rs`](src/lib.rs), [`src/parse/mod.rs`](src/parse/mod.rs), [`src/parse/gguf_write.rs`](src/parse/gguf_write.rs), [`src/parse/pth.rs`](src/parse/pth.rs), [`src/lethe/mod.rs`](src/lethe/mod.rs), [`src/lethe/bnb.rs`](src/lethe/bnb.rs), [`src/lethe/round_trip.rs`](src/lethe/round_trip.rs)) still say `Phase 7.5` where they mean the encode-kernel completion that is now [Phase 8.5](#phase-85-lethe-encode-completion), as do 3 files under `docs/`. The crate contradicts itself in user-facing text: `src/cli.rs` line 79 tells `--help` readers that quantised `GGUF` emit is *"deferred to Phase 7.5"* while [`src/convert.rs`](src/convert.rs) lines 66 and 88 say Phase 8.5, and one `gguf_write` error string ships `"requires Phase 7.5 encoders"` to end users (asserted on by its own test, so the test moves with it). All of it renders on docs.rs. Also add the `## [Unreleased]` section [`CHANGELOG.md`](CHANGELOG.md) currently lacks: it is the external audit's standing observation, and release-checklist step 3 has nothing to rename without it. *(Two stale lines in this file were corrected when this phase was written rather than deferred into it: the status header's "pending release as v0.7.5", and Phase 7.5's tail claiming its changes sit uncommitted.)* — **commit** — **PUSH + tag `v0.7.6`**

**Outcome (shipped as `v0.7.6`, 2026-08-21).** All ten items are done, in eight
commits on `phase-7.6`. Seven results worth carrying forward:

1. **The measured gap was smaller than the audit's figure, and that is the
   point of re-measuring.** Item 1 shipped at **1.24×** (`SmolLM2-135M-Q6_K`,
   241 → 194 ms) and **2.23×** (`Qwen2.5-1.5B-IQ2_M`, 5854 → 2631 ms), medians
   of 5 with `target-cpu=native`, against the audit's best-of-3 default-build
   1.39× / 2.35×. Output is `SHA-256`-identical before and after on both, so
   this is the same computation on more threads. Recorded as Experiment 16.
2. **Items 2 and 7 turned out to be one API decision and shipped as one
   commit.** A dtype-aware estimate and a caller `ParseLimits` are two knobs on
   the same call; giving each its own entry point would have meant a 2×2
   explosion across four formats. `InspectOptions` carries both, and there is
   one `_with_options` per inspect. The cost is that `InspectOptions` is no
   longer `Copy` (it holds a `ParseLimits`, which is `Clone`-not-`Copy` by
   design) and the family takes `&InspectOptions`; callers add one `&`.
3. **The `GGUF` estimate is exact, not approximate.** It is asserted equal to
   the tensor-data byte count `remember` then writes, at all three widths —
   which is the bar for a figure a host gates on, and a stronger claim than the
   phase asked for.
4. **`#![deny(warnings)]` caught a silent-orientation bug in item 8.** The first
   Fortran-order version called `to_c_order` unconditionally; the build failed
   with *"field `fortran_order` is never read"*, which is precisely a
   transposition applied to every C-order array in the world: plausible numbers
   in the wrong places, not a crash. The lint floor earned its keep.
5. **Per-axis builds caught three feature-gate slips the union build could
   not.** A `#[cfg(feature = "pth")]` that migrated onto the wrong function
   while a neighbour was inserted; `SliceSource` / `strip_archive_prefix` gated
   `pth`-only when the shared detector needs them under `npz`; and
   `write_safetensors_bytes` dead under `--no-default-features`. Same discipline
   the MSRV job exists for, applied to features.
6. **The rustdoc feature-combo sweep caught four broken intra-doc links** that
   `--all-features` alone could not see, including one genuinely ambiguous
   `[`parse`]` (both a function and a module) now spelled `fn@crate::parse`.
7. **Two items were resolved by deciding rather than coding, and both are
   recorded in the source.** The `parse()` naming asymmetry is documented on
   `Format` rather than fixed, because renaming the library function would break
   every consumer for a cosmetic gain. And `convert`'s in-memory form *was*
   built (item 3's second half) once the `NPZ` byte parsers made all four
...
`convert_bytes` shipped rather than being deferred, which the plan left open.
Nothing was dropped.

**Deliverable:** `anamnesis` v0.7.6 — every verb has one shape across all four formats: `remember` is a library operation for `GGUF` as it already is for safetensors and `.pth`, `parse` accepts bytes and readers for `NPZ` as it already does elsewhere, `inspect` answers *"how big at the width I intend"* for every format rather than for one, the types the crate returns can grow a field without a breaking change, and a long-running call can be cancelled. [Phase 8](#phase-8-python-bindings-pyo3) then writes bindings over a settled surface instead of deciding Rust API inside PyO3 glue. — **PUSH + tag `v0.7.6`**

**Explicitly out of scope for v0.7.6:**

- **Per-tensor streaming output.** Item 1 rewrites the path that would benefit most and item 5 adds a hook to the same long-running calls, but neither bounds peak heap: `convert`'s hub, `ParsedModel::remember`'s dequantised set, and the `GGUF` `remember` loop are all whole-model resident. Streaming stays [Phase 10](#phase-10-streaming-output); this phase only forbids doc comments that imply otherwise.
- **The encode surface (`forget()` / `TargetKernel`).** [Phase 8](#phase-8-python-bindings-pyo3)'s second bullet already owns that decision, and its recommended option (c) needs nothing from this phase.
- **Zero-copy `NumPy` arrays.** Deferred by [`docs/python-interop.md`](docs/python-interop.md), and unchanged by anything here.

**New dependencies:** None.

### Phase 7.7: Settle each suppressed site on its own measurement

**Goal:** Retire the `clippy::chunks_exact_to_as_chunks` question by **measuring every suppressed site and letting each number decide**, rather than by migrating them on a shared theory. Ships as **`v0.7.7`**, which closes the 0.7 line.

**This phase was re-scoped mid-flight, and the reason is the most useful thing in it.** It began as "finish the migration across the five remaining sites, on the mechanism the `GPTQ` spike proved". Item 1 was placed first as a deliberate falsification gate, on the reasoning that `AWQ` is `GPTQ`'s structural twin and should reproduce its result. **It did not.** `GPTQ` gained `9.87 %` from the change; `AWQ` lost about `45 %` from the identical change. The premise was 1 for 2, with the failure larger than the success, so the phase stopped predicting and started measuring.

| | `GPTQ` (item 0) | `AWQ` (item 1) |
|---|---|---|
| Source change | four-stream migration | four-stream migration |
| `BF16`, x86-64 paired | *(not measured; predates the harness)* | **+48.8 / +49.7 / +43.1 %** |
| `BF16`, aarch64 walltime | **-9.87 %** | **+30.6 %** |
| aarch64 instruction count | fell | **fell** |
| Verdict | kept | **suppression kept** |

**Three instrument failures had to be fixed before any of that could be seen**, and each was found by the item it blocked:

1. **The benchmarks could only see `BF16`.** Every dequant group called a `_to_bf16` entry point, so the arbiter was blind to `F32` and `F16` — two thirds of the `OutputElement` surface. Fixed in item 1a: 13 new arms, 9 to 22 ids.
2. **Two benchmarks were reporting a false 34 % regression** on pull requests that changed no `src/` file, and their two measurement bases disagreed by ~2x. Fixed in item 11.
3. **The arbiter measured the wrong architecture.** CodSpeed's walltime instrument is `aarch64`-only, with no self-hosted option; the crate is developed and released on x86-64. Fixed by the new harness below.

**The instrument now, and its measured floor.** [`benches/ab.rs`](benches/ab.rs) is a *paired* harness built on `tango-bench`: both versions are loaded together and interleaved sample by sample, so drift applies to both sides and cancels. It runs **on x86-64, locally, in about 30 seconds**. Calibrated against an export of byte-identical source, because an instrument whose floor is unknown is not an instrument:

| Instrument | Floor on identical code | Architecture | Turnaround |
|---|---|---|---|
| **`benches/ab.rs` (paired, tango)** | **~2 %** | **x86-64** | **~30 s** |
| CodSpeed walltime | ~4 % | `aarch64` only | ~18 min |
| Local criterion (pointwise) | 10-23 % | x86-64 | minutes |

Three arms earned a significance star while nothing had changed, so **a `*` means "significant given the sampling", not "real"**. Anything under ~2.5 % is no signal; 5 % is the threshold worth acting on.

**And the disassembly is demoted with it.** Instruction counts fell on `aarch64` at *every* `AWQ` width while wall clock rose ~30 %, and fell on x86-64 at two widths of three while wall clock rose ~45 %. **A static instruction count is not a proxy for wall clock, not even on the architecture being timed.** It explains a result; it never decides one. That is the phase's most transferable finding and it belongs in `CONVENTIONS.md` (item 10) regardless of what else ships.

**What this means for the remaining sites.** The four-stream migration is *unpredictable per kernel*: it helped `GPTQ` and hurt `AWQ`, and the only structural difference between them is `AWQ`'s `AWQ_ORDER` scatter in pass 1. So no site may be migrated on the strength of another's number. That was an expensive rule when each answer cost 18 CI minutes on the wrong architecture; at 30 seconds on the right one it is simply the cheap and correct way to work. **The remaining items are experiments, not commitments, and a suppression that survives its own measurement is a result rather than a failure.**

#### Settled

- [x] **0. `GPTQ`, and the `OutputElement::Tile` machinery.** `Tile` / `split_tiles` / `write_tile` landed with `GPTQ`'s four-stream migration at `-9.87 %` on `aarch64`; that module carries no suppression. Shipped after the `v0.7.6` tag, so it releases as part of `v0.7.7` and is recorded under `## [Unreleased]` in [`CHANGELOG.md`](CHANGELOG.md). **Its number survives the re-scope; the reasoning quoted beside it does not** — it was a `BF16`-only measurement partly justified by instruction counts, which item 1 then showed can point the wrong way — **`6550c26` / `b5be59b`, merged `de9f390`**
- [x] **1a. The benchmarks could only see `BF16`.** All six migration-relevant groups gained `_f32` and `_f16` arms, plus an `_f16` arm for `dequant_gguf_q4_k`, whose output goes through the `write_scratch` item 2 rewrites. The arms **record rather than gate**: `BF16` stays each group's guard, being the default width and the one every historical number is quoted at — **merged `b925dbc`**
- [x] **11. Two benchmarks were poisoning the instrument.** The file-writing groups cannot see the parallel path (`167.45` vs `168.47 ms` across thread budgets; `315.39 ms` at *both* at `F32`), and their two measurement bases disagree by ~2x. They now sit behind a `bench-fileio` feature CI does not enable, and still run under `cargo bench --all-features`. `convert_bytes_gguf_to_safetensors` remains the real CI guard at a genuine **4.62x** — **merged `3807c88`**
- [x] **1. `AWQ` is settled: the migration is slower, and the suppression stays.** Measured on both architectures, reproduced three times on x86-64. The comment in [`src/remember/awq.rs`](src/remember/awq.rs) now records what was measured rather than what was predicted, and says plainly that both disassemblies pointed the wrong way — **commit**
- [x] **13. The instrument itself.** [`benches/ab.rs`](benches/ab.rs) added, with its floor calibrated and written down; CodSpeed demoted from judge to watchman by removing its `pull_request` trigger, and its workflow header now states what `aarch64` bare metal is **not** — not Apple Silicon (different microarchitecture, `ARMv8.2+` hardware `FP16`, macOS not Linux) and not the whole story — **commit**

#### Open, each now a 30-second experiment

- [x] **2. `write_scratch`, the shared narrowing writer. MEASURED, NOT TAKEN.** Migrating all three narrowing loops, paired harness on x86-64, two runs: `bnb_int8` `BF16` **+37.7 % / +31.7 %** and `F32` **+17.8 % / +13.4 %**, against `gptq_int4` `F16` **-10.6 % / -10.0 %**. Everything else moved inconsistently between runs, **including every `gguf_q4_k` arm** — worth recording, because all 24 `GGUF` kernel functions reach their output through this writer and yet showed nothing. A +32 % regression on the default width of one kernel is not paid for by a 10 % gain on the least-used width of another. The suppression stays, now on its own attribution rather than inherited from the `FP8` family revert — **commit**
- [x] **3. The `FP8` family. MEASURED, NOT TAKEN — and v0.7.6's figure was overstated.** `fp8_fine` `BF16` **+3.21 % / +4.56 %**, reproduced, with no effect anywhere else including the `per_tensor` arms that share the same function. So migrating this module costs about **+3 to +5 %, not the +18 % to +21 %** v0.7.6 recorded: that larger figure was a pointwise reading at a 10-23 % floor and was mostly noise. Still a measured loss with no compensating gain, so the loop stays — but the number is now small enough to be worth re-testing if the surrounding code changes shape — **commit**
- [x] **4. The `BnB` family. MEASURED AND TAKEN — the phase's one migration win beyond `GPTQ`.** `bnb_int8` at `F16`: **-5.22 %, -2.87 %, -5.96 %** across three runs, all significant at a 2.5 % threshold, with no reproduced regression at any width or in any other family. v0.7.6 had declined this *despite* a favourable -7.6 % reading, correctly, because that sat inside an instrument whose error the same run demonstrated at +10.6 % on byte-identical source. A ~2 % floor resolves it. `bnb.rs` now carries **no suppression at all**, and its `VECTORIZED:` annotation was re-read against the new loop shape rather than left describing the old one — the counts are no longer uniform across `E` and are now listed per width. Bit-exact: 700 tests pass — **commit**
- [x] **5. `convert.rs::to_bf16_bytes`. UNMEASURABLE, and recorded as that rather than as "measured slow".** The function is private, so the bench crate cannot call it; the only public path that reaches it (`convert_bytes` into `ConvertTarget::BnbNf4`) also parses, builds the hub and runs the `NF4` encode, in which this loop is a small enough fraction that a 5 % change lands below the harness's own ~2 % floor. **A benchmark that cannot resolve the effect is not evidence**, and building one anyway would have dressed a guess as a measurement. No inference is available from the measured sites either, since this phase's whole finding is that the migration is unpredictable per site. To settle it, give the narrowing a callable seam rather than a bigger benchmark — **commit**
- [x] **6. The fourteen test and bench crates. SWEPT ENTIRELY — the rationale did not describe the code.** Their blanket `#![allow]` rested on the claim that they *"mirror the kernel loops [they] exercise"*. **They do not.** Every one of the 35 sites is a cold assertion loop decoding output bytes (`chunks_exact(2)` over `BF16` pairs, `chunks_exact(4)` over `F32` words) or a fixture builder, at a **concrete** width — never `VECTOR_TILE * E::BYTES`, and mirroring no hot loop at all. So there was nothing to preserve: all 14 migrated, all 14 suppressions deleted, and the `#![allow(unknown_lints)]` MSRV guards carried alongside them went too, having had no other referent. Verified on MSRV 1.88 without them — **commit**
- [x] **7. v0.7.6's comments corrected, and its internal contradiction resolved.** Each of the six sites now states what was measured, on which architecture, with which instrument (items 1-5). The `FP8` contradiction is settled by measurement rather than by picking a side: `fp8.rs` claimed the regression was *"isolated to this module"*, `output.rs` that it *"could not be isolated away from here"*, and measuring each separately shows **both were overstating** — the module's own cost is ~+3-5 %, not +18-21 %. Also corrected here: [`src/remember/gptq.rs`](src/remember/gptq.rs) still predicted that `AWQ` *"should follow"* and cited falling instruction counts as corroborating evidence. `AWQ` refuted the first and item 1 refuted the second — **commit**
- [ ] **8. RETIRED, and recorded rather than dropped.** This item asked us to attribute a consistent `-4.16 %` that `dequant_gguf_q4_k` showed on CodSpeed while untouched. It existed to protect a number CodSpeed was about to adjudicate. CodSpeed no longer adjudicates anything (item 13), and the effect was its own layout noise on an architecture we do not release from, so the question no longer gates a decision. **It is not resolved, only de-prioritised**, and the `~4 %` figure it produced survives as that instrument's measured floor in the table above — **no commit**
- [x] **9. `docs/perf-experiments.md` Experiment 17.** The per-site table, the three measured instrument floors, the three instrument defects found on the way, and the finding that outlives the phase: an instruction count is not a proxy for wall clock. Written up as a *negative* result in the majority — 3 losses, 1 unmeasurable, 2 wins — which is the point of that file — **commit**
- [x] **10. [`CONVENTIONS.md`](CONVENTIONS.md) amended, and the *Benchmark evidence* section rewritten around which instrument may decide what.** Its rules go 5 to 9, adding: **(6)** state the instrument's floor and measure it against identical source, with the three floors tabulated and the warning that a significance marker means "significant given the sampling", not "real"; **(7)** a static instruction count never decides a perf question, on any architecture, with `AWQ`'s numbers as the worked example; **(8)** the architecture a number came from is part of the number; **(9)** measure the site you changed, not a sibling, with untouched kernels as controls. `MEASURED-REVERT` grows from four required parts to six — instrument and architecture, and *what shape was measured* — and **"unmeasurable" is established as a legitimate verdict** that must not be written as "measured slow" — **commit**
- [x] **12. `F16` output costs 2x to 3x `BF16` at the same width — on BOTH architectures, which the plan had wrong.** This item was scoped as an `aarch64` finding with Apple Silicon as the open question. Measuring x86-64 with the paired harness shows **the same penalty there**: `gguf_q4_k` 25.70 to 79.87 ms (**3.11x**), `bnb_int8` 24.84 to 76.95 ms (**3.10x**), `gptq_int4` **2.71x**, `awq_int4` **2.65x**, `fp8_fine_grained` **2.48x**, `bnb_nf4` **2.47x**, `fp8_per_tensor` **2.02x**, against `aarch64`'s 2.10x-2.93x on the same seven kernels (criterion medians, not the paired harness — see below). **Deliverable (a) is answered in the negative, and usefully so:** the `aarch64` inlining failure is *not* the lever, because x86-64 already has the inline and still pays 2x to 3x. Two different mechanisms give the same magnitude — x86-64's `vcvtps2ph` takes a 128-bit source, so the narrowing runs **4 lanes** where `BF16`'s bias-and-shift runs **8**; `aarch64` simply does not inline `F16Out::write_scratch` at all. The conversion is the cost, not the call, so no cheap fix exists and none is attempted. **Deliverable (b) ships:** [`F16Out`](src/remember/output.rs) gains a `# Cost` section with the table, both mechanisms, and the Apple Silicon question left explicitly open. [`F32Out`](src/remember/output.rs) is quantified in passing too — it costs **1.09x to 1.63x**, *less* than its 2x byte ratio implies, because it skips the narrowing `BF16` performs. **And a methodological correction was needed to get these numbers:** the first cut read absolute times off the paired harness, which samples adaptively for *differences between binaries* and whose printed magnitudes swing 50-100 % run to run. A width ratio is a *within-binary* comparison with no layout term, so a criterion median is the correct statistic. **A paired harness answers "is A faster than B", not "how long does A take"** — **commit** — **PUSH + tag `v0.7.7`**

**Deliverable:** `anamnesis` v0.7.7 — every `chunks_exact_to_as_chunks` suppression in `src/` either deleted on its own measurement or kept with a comment recording what was actually measured, on which architecture, with which instrument; a paired x86-64 harness that answers the next such question in 30 seconds; and a `CONVENTIONS.md` that no longer lets an instruction count stand in for a stopwatch. — **PUSH + tag `v0.7.7`**

**Outcome (shipped as `v0.7.7`, 2026-08-22).** Every item is done except the retired one. **The phase answered its question in the negative and is more valuable for it.** It set out to finish a migration on a shared theory; it ends having established that the theory does not generalise, having found and fixed three defects in its own measuring apparatus, and having retired a rule the project had been relying on. Eight results worth carrying forward:

1. **Of six suppressed sites, two migrations are wins, three are measured losses, one is unmeasurable.** `GPTQ` `-9.87 %` and `BnB` `INT8` `-3` to `-6 %` at `F16` shipped; `AWQ`, `write_scratch` and `FP8` kept their suppressions on their own numbers; `convert.rs::to_bf16_bytes` is recorded as **unmeasurable**, which is a different claim from "measured slow" and is now a permitted verdict in [`CONVENTIONS.md`](CONVENTIONS.md).

2. **Identical source changes to near-identical kernels gave opposite answers.** `GPTQ` and `AWQ` differ only by `AWQ`'s `AWQ_ORDER` scatter; the same four-stream migration gained one 9.87 % and cost the other ~45 %. **No site may be migrated on a sibling's number**, and that is the phase's central finding.

3. **A static instruction count never decides a perf question**, on any architecture, including the one being timed. `AWQ`'s `aarch64` counts fell at *every* output width while its wall clock rose ~30 %. The rule this phase started under — "neutral ships only if the disassembly improves" — was retired by its own evidence. Instruction counts explain; they never adjudicate.

4. **The measuring apparatus was wrong in three independent ways, each found by the item it blocked.** The benchmarks could see only `BF16`, two thirds of the `OutputElement` surface being invisible (item 1a, 13 arms added). Two file-writing benchmarks reported a false 34 % regression on pull requests touching no `src/` file, and disagreed with themselves by ~2x between measurement bases (item 11). And the arbiter measured `aarch64` while the crate is developed and released on x86-64.

5. **Every instrument must state its floor, measured against byte-identical source.** Paired harness ~2 %, CI walltime ~4 %, local pointwise criterion 10-23 %. A significance marker means "significant given the sampling", not "real": the paired harness starred three arms while nothing had changed. v0.7.6's `+18 %` to `+21 %` `FP8` figure was mostly floor — the real cost is `+3` to `+5 %`.

6. **Tools answer different questions, and using one for the other is a defect.** [`benches/ab.rs`](benches/ab.rs) resolves *differences between binaries* and its absolute magnitudes swing 50-100 % between invocations; a width *ratio* is a within-binary comparison with no layout term, so it wants a criterion median. This phase published a table from the wrong one and corrected it, which is why both harnesses now say in their own docs what they may and may not be quoted for.

7. **`F16` output costs 2.02x to 3.11x `BF16` at identical output width**, on both architectures — slower even than `F32`, which writes twice the bytes, because the cost is the conversion and not the traffic. Nothing had ever said so: `F16Out`'s documentation discussed range and precision at length and speed not at all. It now carries the table, and so do the CLI reference, the FAQ and the output-dtype tutorial.

8. **One change ships knowingly imperfect, and says so in three places.** `BnB` `INT8` at `F16` is ~5 % faster on x86-64 and ~21 % slower on `aarch64`. The `aarch64` hardware reachable here is Linux server-class ARM *without* hardware `FP16`; Apple Silicon has `ARMv8.2 FEAT_FP16` and is unmeasured. Reverting on a proxy would be as unevidenced as shipping on one, so it ships disclosed — in [`src/remember/bnb.rs`](src/remember/bnb.rs), in [`CHANGELOG.md`](CHANGELOG.md) with a recommendation to pin `0.7.6`, and in [issue #11](https://github.com/mi-for-the-rust-of-us/anamnesis/issues/11) with a reproduction, a calibration step and a thermal-throttling warning for laptops.

**Deviations from the plan, stated plainly:** item 1a (the width arms) and item 13 (the harness and CodSpeed's demotion) were not in the plan and were added when the items they blocked surfaced them; item 8 was **retired rather than completed**, because it existed to protect a number the demoted instrument was about to adjudicate; item 5's planned `convert_bytes` path benchmark was *not* built, because it could not have resolved the effect and building it would have dressed a guess as a measurement. Nothing was dropped silently.

**Deliverable:** `anamnesis` v0.7.7 — every `chunks_exact_to_as_chunks` suppression in `src/` is either deleted on its own measurement or kept with a comment recording what was measured, on which architecture, with which instrument; the fourteen test and bench crates carry none at all, their stated rationale having been false about the code it guarded; and the project has a paired x86-64 harness that answers the next such question in 30 seconds. — **PUSHED + tagged `v0.7.7`**

**Explicitly out of scope for v0.7.7:**

- **Migrating a site because a sibling improved.** The whole finding is that this does not transfer. `GPTQ` kept its migration and `AWQ` did not, from identical source changes.
- **Anything that changes output bytes.** Every item is a codegen-shape change behind unchanged arithmetic; the `cross_validation_*` suites are the gate.
- **Chasing the `AWQ_ORDER` explanation.** It is the only structural difference between the twins and therefore the likely cause, but confirming it is a research task, not a release item. Record the hypothesis; do not spend the tag on it.
- **A continuous x86-64 service.** [Bencher](https://bencher.dev) plus a dedicated runner would reconstruct CodSpeed on the right architecture, and its generic JSON adapter accepts external measurements where CodSpeed's action has no such input at all. Worth its own decision, not this tag's.

**New dependencies:** `tango-bench` (**dev-dependency only**, so no consumer of the crate acquires it). Verified to build on MSRV `1.88`.

### Phase 8: Python Bindings (PyO3)

**Goal:** Expose anamnesis to the Python ecosystem via `pip install anamnesis-quant` (see Step 0 for why the distribution name differs from the crate). Phases 1–7 give anamnesis the fastest dequantization (2.7–54× vs PyTorch), the fastest NPZ parser (17.7× vs npyz), PyTorch `.pth` parsing, GGUF support, BnB encoding, the `convert()` pipeline scaffold, and — from the immediately preceding [Phase 7](#phase-7-cpu-simd-pass) — multi-threaded dequant whose ~3–4× speedup **survives wheel distribution** (threads work regardless of the wheel's `target-cpu`, unlike compile-time SIMD). Python bindings multiply the audience by ~100×, replacing ad-hoc dequantization scripts across the ML community. By shipping after the format conversion scaffold, `pip install anamnesis` exposes `convert()` from day one; the remaining encode-side targets (GGUF / FP8 / IQ / TQ / MXFP4) light up at v0.8.5 / Phase 8.5 through the same Python API once those kernels land.

**Approach:** Use [PyO3](https://pyo3.rs/) + [maturin](https://github.com/PyO3/maturin) to build a native Python extension. The Python API should mirror the Rust library API closely: `parse()`, `inspect()`, `remember()`, `convert()`, `parse_npz()` (on `forget()`, see the prerequisite audit below — it does not exist yet). Returns NumPy arrays (via `numpy` interop) or raw bytes, following the ownership / `BF16` contract frozen in [Phase 6.13](#phase-613-python-readiness-hardening) Step 4 **and restated for the caller-chosen dtype in [Phase 7.4](#phase-74-caller-chosen-output-dtype-remember-path)**. Ships as a wheel on PyPI.

**Prerequisite audit (performed 2026-08-15, immediately after the v0.7.4 release).** Phase 6.13 was explicitly a Python-readiness phase, and its guarantees were re-verified green against the released tree rather than assumed:

| Prerequisite | Where it lives | Verified by |
|---|---|---|
| `[profile.python]`, `panic = "unwind"` | `Cargo.toml` | `tests/panic_profile.rs` (3) |
| Copy-based, mmap-free parse paths (no uncatchable `SIGBUS`) | `parse_bytes` / `parse_*_from_reader` | `tests/parse_owned_path.rs` (8) |
| Owned extraction outlives a dropped `Parsed*` | `Cow::into_owned` on every borrow path | `tests/python_ownership_contract.rs` (4) |
| Never panics on hostile input | every parse entry point | `tests/no_panic.rs` (5) + `cargo fuzz` |
| Error hierarchy to map onto | `Parse`, `Unsupported`, `LimitExceeded`, `DisallowedGlobal`, `Io` | all five present in `src/error.rs` |
| `ParsedModel: Sync` (needed to release the GIL and still thread) | `src/model.rs` | `tests/parallel_contract.rs` |
| Caller-chosen output dtype on **both** paths | `TargetDtype`, `ConvertOptions::with_output_dtype` | v0.7.3 + v0.7.4 |

**Two gaps the audit found, both of which change this phase's scope:**

1. **`ParsedModel::forget()` and `TargetKernel` do not exist.** The encode surface today is free functions (`encode_bnb4`, `encode_bnb_int8`, `write_bnb_nf4_safetensors`, …) plus `ConvertTarget::BnbNf4` on the `convert` path. [Phase 8.5](#phase-85-lethe-encode-completion) step 7 says it will *extend* a `forget()` dispatch "introduced in Phase 5" — Phase 5 introduced neither. Each phase assumed the other had built it. Resolved below by making the decision explicit rather than by inheriting a non-existent API.
2. **This phase's bullets predated v0.7.3 / v0.7.4** and described a `BF16`-only world in which `ml_dtypes` is the main path. That is now inverted: a caller asks for `f32` and needs no optional dependency at all, which was the entire payoff the 7.3 / 7.4 pair was meant to buy. The bullets below are rewritten accordingly.

**A later audit, 2026-08-20 against `31cb17e`, found ten more, and they are not in this phase.** Where the 2026-08-15 audit and the external 2026-08-17 one both checked *hardening* prerequisites and found them green, the third checked the **shape of the public API** this phase has to mirror, and found it grew per-format: `GGUF` `remember` exists only inside the CLI, `NPZ` has no bytes or reader parse, a dequantised-size estimate exists for one format in four, twenty returned structs cannot grow a field without a breaking change, and nothing can be cancelled once the GIL is released. All ten ship first, as [Phase 7.6](#phase-76-api-parity-before-the-bindings) / `v0.7.6`, for the reason this phase's own encode-surface bullet gives: Rust API decided under Python deadline pressure is how a PyPI signature goes wrong permanently. **Read that phase before starting Step 0** — items 2, 5, 6 and 7 there settle questions several bullets below currently assume are already settled.
...
- [ ] **Step 0 — write the pitch, before any code and before the API decisions** (`docs/python-pitch.md`, ~half a page) — **this is deliberately the first bullet, and the ordering is the point.** Every other phase in this roadmap starts with code because the audience was already known: Rust consumers with a problem anamnesis uniquely solves. Phase 8 does not have that luxury, for a reason recorded plainly below. The pitch must answer four questions in a form a stranger can read in a minute: **who is this for, what problem do they have today, what do they type, and why not just use Python.**
  ...
  **The load-bearing caution: the Rust argument does not transfer.** [`docs/rust-ecosystem-comparison.md`](docs/rust-ecosystem-comparison.md) establishes that anamnesis is *unique* in Rust — and it excludes Python tooling **by design**, in its own Scope line. On PyPI the competition is `bitsandbytes`, `GPTQModel`, `AutoAWQ` and `gguf-py`: all of which exist, work, and are what people already have installed. **Phase 8 moves anamnesis from "the only option" to "a better option", which is a bigger market and a harder sell.** Do not reuse the matrix's framing; it will read as a Rust-crate argument to a Python audience and land as nothing. Build the pitch on the three things that survive the move:
  - **One API across five families** (`FP8` / `GPTQ` / `AWQ` / `BnB` / `GGUF`) instead of four libraries with four conventions.
  - **Measured speed against the tool they would otherwise use** — 2.7–54× vs PyTorch CPU, 1.11–6.75× vs the closest Python default on `convert`. Benchmarked against the real alternative, not a strawman.
  - **Exactness, stated as the counterintuitive fact it is:** anamnesis on CPU is bit-identical to `bitsandbytes`' **CUDA** kernel, whose own **CPU** kernel double-rounds and diverges by 1 `ULP` on ~19 % of elements. *GPU-kernel-exact results without a GPU*, and more exact than the reference library's own CPU path. This is measured and committed (`tests/fixtures/bnb_reference/generate_bnb.py`), and it is the single most attention-earning claim available.

  **Pre-empt the CPU perception problem explicitly.** An ML audience reads "CPU-only" as "slow" regardless of workload shape, so the pitch must say why it is a design choice: dequantisation is **memory-bandwidth-bound, not compute-bound** (Experiments 10, 11, and v0.7.4's 2.03×-vs-3.75× threading result all measure this), the dataflow is disk → RAM → disk so a GPU round trip adds PCIe transfers that typically cost more than the arithmetic they accelerate, VRAM is the binding constraint anyway (a 70B model at `F32` is ~280 GB, more than any single GPU), and a CPU-only wheel installs on every runner, container, laptop and ARM Mac with no CUDA matrix. Note the coherence rather than leaving it to look accidental: the one place a GPU genuinely helps is calibration-based encode (`GPTQ`/`AWQ` Hessians and activation statistics), which [Phase 8.5](#phase-85-lethe-encode-completion) already puts out of scope for exactly that reason. Left implicit, "CPU-only" reads as an apology instead of a decision.

  **Distribution name, decided 2026-08-15: `anamnesis-quant`** (import `anamnesis_quant`). Not a preference — **`anamnesis` is already taken on PyPI** (v1.0.4, "Object serialisation to/from HDF5 and via MPI"), as are `lethe` and `amn`, so a distinct distribution name is forced. `anamnesis-quant` keeps the brand, avoids shadowing the existing package, and buys the search term users actually type; `-py` / `-pyo3` suffixes were rejected as implementation details with no keyword surface. The Rust crate keeps the name `anamnesis` unchanged.

  The output is the file the wheel ships as its README and PyPI `long_description`, so it is written once and reused rather than restated — **commit**
- [ ] **Decide the encode surface before any binding is written** — the audit above found no `forget()` to bind. Three options, and the choice is load-bearing because PyPI punishes signature churn far more than crates.io does: **(a)** build `ParsedModel::forget(TargetKernel)` now, in Rust, and bind it (moves Phase 8.5 step 7's dispatch work forward into this phase); **(b)** bind only `convert(..., target="bnb-nf4")`, which is the encode path that genuinely ships today, and add `forget()` to Python at v0.8.5 alongside the kernels that make it worth having; **(c)** bind `forget()` now with a deliberately narrow `kernel="bnb-nf4"` argument, accepting that every 8.5 kernel widens the accepted value set but never changes the signature. **Recommendation: (c)**, because it is the only one that neither invents Rust API under Python deadline pressure nor forces a breaking change at v0.8.5 — a widening enum of accepted strings is additive. Whichever is chosen, record it here and fix [Phase 8.5](#phase-85-lethe-encode-completion) step 7's claim that Phase 5 introduced the dispatch — **commit**
- [ ] PyO3 module scaffold (`src/python.rs`) — feature-gated behind `python`; `crate-type = ["cdylib", "rlib"]` (the `rlib` so the existing Rust test suite still links against the same crate) — **commit**
- [ ] `parse_npz()` binding — returns `dict[str, NpzTensor]` with NumPy-compatible arrays — **commit**
- [ ] Safetensors `parse()` + `inspect()` bindings — `inspect()` takes the **output dtype** and returns the estimate for *that* width, mirroring `InspectOptions::with_output_dtype` (v0.7.4). This is not cosmetic: `dequantized_size` feeds the [Phase 6.8](#phase-68-untrusted-input-resource-governance) inspect-before-parse gate, and a `BF16` estimate understates an `f32` request by exactly 2×. v0.7.4 shipped that very bug in the CLI (`amn remember --to f32` reported the `BF16` size) and fixed it; do not reintroduce it in Python. The returned object must also carry the dtype the figure assumed, so a number can never be read without its width — **commit**
- [ ] **`remember()` / `convert()` bindings, with the output dtype as a first-class parameter** — `remember(path, to="bf16"|"f32"|"f16")` and `convert(path, to=<format>, out_dtype=...)`, matching the Rust spelling on each path (`--to` selects a dtype on `remember` because there is no format to choose; `--out-dtype` on `convert` because `--to` is already the format). The returned array's dtype must reflect what was **actually produced**, never a fixed assumption. Passthrough tensors keep their source dtype, so a `remember()` result is legitimately a **mixed-dtype mapping** and the binding must surface it as such rather than flattening it — flattening would either invent precision or discard it. No "give me whatever is cheapest" mode: the caller chooses, or gets the documented `bf16` default — **commit**
...
- [ ] **Wheel baseline / multiversioning decision — `x86-64-v3` primary + SSE2 fallback:** a PyPI wheel cannot use `target-cpu=native`, so a default x86-64 wheel ships the SSE2 baseline and loses the AVX2 throughput the headline numbers are measured at. Build the **primary x86-64 wheel for the `x86-64-v3` micro-architecture level** (`-C target-cpu=x86-64-v3` → AVX2 / FMA / BMI via auto-vectorisation across *every* kernel, not just the Phase 7 pass-2 writer), and ship a **second `x86-64-v1` (SSE2) fallback wheel** for pre-2013 CPUs; document the support policy ("requires AVX2; SSE2 fallback wheel available") in the README and wheel metadata. Note Phase 7 shipped **multi-threading, not a runtime SIMD dispatcher** (explicit SIMD measured null — Experiment 10), so the SSE2 fallback wheel gets **no auto-AVX2 rescue** on vectorisable code — it is simply slower there. What *does* carry across every wheel is the **~3–4× threading win**, which is independent of `target-cpu`, so even the v1 fallback wheel gets the phase's headline speedup. **ARM / aarch64** wheels need no special handling — NEON is baseline in aarch64, so auto-vectorisation applies automatically and the threading win is architecture-independent.

  **Qualify the threading number before repeating it on PyPI.** The "~3–4×" is measured on the `remember` path and holds there (CodSpeed, v0.7.4: `remember_bf16_whole_model` 78.17 → 20.83 ms, **3.75×**). It does **not** hold end-to-end on `convert`: the same run measured `convert_gguf_to_safetensors` at **1.00×** from 1 to 4 threads (167.60 → 168.41 ms), where [Experiment 12](docs/perf-experiments.md) recorded 1.26–1.36× locally. The likely cause is that the output write is ~60 % of that path's wall clock (Amdahl) and CI storage is slower still, but it is unexplained rather than explained. **Resolve it or qualify Experiment 12 before a PyPI page advertises a single blended figure**, and note the third case while you are there: `F32` scales at only **2.03×** against `BF16`'s 3.75×, because twice the output bytes hits the memory-bandwidth ceiling sooner. One number does not describe all three paths — **commit**
- [ ] `maturin` build config, PyPI packaging, CI workflow — the wheel build selects `--profile python` (`panic = "unwind"`), which `tests/panic_profile.rs` already guards; a wheel built on `release` would be an uncatchable-abort regression that no Rust test can see, so assert the profile in the workflow itself — **commit**
- [ ] **Python test suite — validate against the same fixtures the Rust suites use, at every output dtype** — the cross-validation corpus lives in `tests/fixtures/` and is **excluded from the published crate** (it ships via the GitHub Release per tag), so the Python tests run from a repo checkout, not from an installed wheel. Assert the three properties the Rust side already pins, now through the binding: output dtype is what was requested; passthrough tensors keep their source dtype; output is byte-identical across thread counts. Add the binding-level hostile-input smoke test (malformed bytes → Python exception, **never** a process exit) called for in the hardening step. **Needs `ml_dtypes` installed** to exercise the `bf16` return path — it is absent from the current dev environment, so add it to the Python dev requirements rather than discovering it mid-step — **commit**
- [ ] **Reference docs — Python FAQ section + `pip`-to-NumPy tutorial (mirroring [hf-fetch-model](https://github.com/mi-for-the-rust-of-us/hf-fetch-model)'s `docs/` system).** *This is the second half of a split: the **positioning** half is Step 0 and lands first, because it gates the API; what remains here is reference material, which can only be written once the API exists.* Do not restate the pitch — link it, and cover what it deliberately omits. Add a **Python section** to `docs/FAQ.md` (install; *"why is the wheel slow on my old CPU?"* → the `x86-64-v3` + SSE2-fallback policy from the wheel-baseline step; which exception to catch, per the [Phase 6.13](#phase-613-python-readiness-hardening) `ParseError` / `LimitExceededError` / `SecurityError` hierarchy; **which dtype to ask for, and why `ml_dtypes` is usually unnecessary**) plus a `docs/tutorials/` walkthrough (`pip install` → `inspect()` at the width you intend → first dequantized NumPy array).

  **The `ml_dtypes` framing is inverted from what this bullet originally said**, and the inversion is the point of the 7.3 / 7.4 pair. The old plan promised an entry on *"how to get a `bf16` NumPy array via `ml_dtypes`"*, written when `BF16` was the only thing a dequantising call could return. Post-v0.7.4 the primary answer is that a NumPy user asks for `f32` and gets a native `np.float32` array with **no optional dependency at all**; `ml_dtypes` is the niche path for callers who specifically want `bfloat16` back, and the no-silent-widening rule still forbids upcasting behind their back. Lead with the choice, not with the workaround. Reuse the [Choosing an output dtype](docs/tutorials/choosing-an-output-dtype.md) tutorial v0.7.4 shipped rather than restating it: the Python chapter should link it and cover only what differs (NumPy dtype mapping, the `ml_dtypes` fallback, the mixed-dtype dict a `remember()` returns). Built on the `docs/python-interop.md` design note from [Phase 6.13](#phase-613-python-readiness-hardening) Step 4, and reusing the FAQ/tutorial conventions (`STYLE CONVENTIONS` block, freshness marker, 30-second answer, bash + PowerShell commands) the sibling hf-fetch-model docs already follow. *(The general `docs/FAQ.md` + `docs/tutorials/` scaffold for Rust/CLI users lands earlier as a non-versioned docs chore; this step adds the Python chapters.)* — **commit** — **PUSH**

**Deliverable:** `anamnesis` v0.8.0 — **`pip install anamnesis-quant`** works, with a pitch that states who it is for before it states what it does, the full conversion matrix exposed, the **caller-chosen output dtype on both the `remember` and `convert` bindings** (so a NumPy user gets a native `np.float32` array with no optional dependency), an `x86-64-v3` wheel that delivers the advertised AVX2 throughput (SSE2 fallback wheel for older CPUs), and Python user docs (FAQ section + `pip`-to-NumPy tutorial). — **PUSH + tag `v0.8.0`**

**New dependencies:** `pyo3`, `numpy` (PyO3 interop), feature-gated behind `python`. The Python-side `ml_dtypes` is **optional and no longer on the common path**: since v0.7.4 a caller who wants a native NumPy dtype asks for `f32` or `f16` up front, and `ml_dtypes` is needed only to return `bfloat16` itself. It remains a dev-requirement for the test suite, which must still exercise that path.

**Explicitly out of scope for v0.8.0:**

- **Zero-copy arrays.** [`docs/python-interop.md`](docs/python-interop.md) defers them deliberately: a NumPy `base` `PyCapsule` holding the owning `Parsed*` is a real copy-elision win and a use-after-free footgun if the lifetime wiring is ever wrong. First wheel copies; zero-copy is a later opt-in, never the default.
- **Encode kernels beyond `BnB`.** They land at [Phase 8.5](#phase-85-lethe-encode-completion) and light up through whatever encode surface the first bullet settles on, without a signature change.
...
- [ ] **Step 7 — `forget()` dispatch + `forget` CLI subcommand + cumulative cross-validation** (`src/lib.rs` + `src/cli.rs`) — **correction, found by [Phase 8](#phase-8-python-bindings-pyo3)'s prerequisite audit on 2026-08-15: neither `TargetKernel` nor `ParsedModel::forget()` exists.** Phase 5 shipped the encode *kernels* (`encode_bnb4`, `encode_bnb_int8`) and the `lethe::bnb_writer` surface, reachable from the high level only as `ConvertTarget::BnbNf4`; it never introduced a `forget()` dispatch for this step to extend. So this step **creates** that dispatch rather than widening it, unless Phase 8 already did so under its own first bullet — check there before starting. Introduce the `TargetKernel` enum and `ParsedModel::forget()` covering every kernel landed in steps 1–6, plus the `amn forget model.safetensors --to <kernel> -o out.gguf` CLI subcommand (alias `quantize`). The cross-validation suite is the cumulative roll-up of every per-step cross-validation test — bit-exactness contract holds at 0 ULP for the codebook-LUT kernels (steps 2–6) and within FP8 representation error for the FP8 path (step 1). Mirrors the v0.4.2 closeout step from Phase 4.5. ~300 LOC, ~2 effort units — **commit** — **PUSH + tag `v0.8.5`**

**Deliverable:** `anamnesis` v0.8.5 — Lethe encode completion. Every block-quant format anamnesis can decode out of can now also be encoded into. Combined with Phase 6's any-to-any conversion CLI and Phase 8's Python bindings, `pip install anamnesis-quant` ships a complete read/write/convert matrix for the GGUF + FP8 + BnB universe — the first time any tool, Rust or Python, has covered all three families end-to-end. — **PUSH + tag `v0.8.5`**
...
- [ ] **GGUF CLI streaming path** — refactor `run_remember_gguf` in `src/cli.rs` to drop the `tensor_data: Vec<(...)>` accumulator in favour of a lazy iterator. Same `LazyDequantView` infrastructure — **commit**
